...
\commentOC{Remove the square after propositions. Maj for propositions, …}
...
\color{blue} MN: I have been reading Moulin (1983)'s book on veto core correspondence (that is Pareto and veto rule). He mentions that for each outcome in the Pareto and veto rule, there is an order of sincere vetoes that isolates it.

Namely,

Consider an order of players $\{1,2,1,2\}$ and the following naive algorithm as follows:\\
\noindent Step 1: player 1 removes his worst $B_1$ in $A$\\
\noindent Step 2: player 2 removes his worst $B_2$ in $A\setminus B_1$\\
\noindent Step 3: player 1 removes his worst $B_3$ in $A\setminus B_1\cup B_2$\\
\noindent Step 4: player 2 removes his worst $B_4$ in $A\setminus B_1\cup B_2 \cup B_3$\\
...
To obtain $e$ again in the example of Remark 5, the sequence $\{1,1,2,2\}$ seems to work. Similarly, to obtain $a$, the sequence $\{2,2,1,1\}$ seems to work.

Finally, to isolate $b$, the sequence $\{1,1,1,2\}$ seems to work.
