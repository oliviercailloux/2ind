...
Théorèmes :
	...
	\end{itemize}
Écrirait-on l’introduction et la conclusion autour de ce plan ?} 
...
\commentOC{I think that defining $\rhoP$ here is not helpful and is one more (unuseful) thing that the reader has to remember. I suggest to replace this sentence with “Note that $\min_{x \in \allalts} \set{\max \lprof(x)}$, that we call the “minimal worst loss”, represents the least loss level at which some alternative receives unanimous support; in other words, $\min_{x \in \allalts} \set{\max \lprof(x)} = \min \set{k \in \losses \suchthat U(\prof, k) ≠ \emptyset}$. See also below the comment that I have added at the sole place where we currently use $\rhoP$.}  \commentRS{OK}

\section{SCRs in the literature}

We start by defining the SCRs of the literature that we analyze in the paper.
\commentOC{… and with some preliminary / basic observations about these rules?} \commentRS{OK}

\textbf{Unanimity compromise} is the SCR $\FB$ that picks all alternatives with unanimous support at $\rhoP$: $\FB(\prof) = U(\prof, \rhoP)$. 
\commentOC{I suggest replacing with “… with unanimous support at the minimal worst loss $\rhoP = \min_{x \in \allalts} \set{\max \lprof(x)}$: $\FB(\prof) = U(\prof, \rhoP)$”.} \commentRS{OK}
\begin{remark}
	\label{th:FBH}
	$\FB$ is anonymous and neutral. It satisfies the Pareto property. Moreover, $\forall \prof$, $\FB(\prof) \subseteq H(\prof)$. The final observation comes from the fact that $\forall \prof$, $\rhoP\leq \khalf$, a consequence of the following proposition that is a restatement of \citet[Theorem 1]{BramsKilgour2001}. We include a proof for completeness.
\commentOC{I suggest to replace $\rhoP$ in the last sentence with $\min_{x \in \allalts} \set{\max \lprof(x)}$, which I believe will help the reader see the link between the statement and the proposition.} \commentRS{OK}
\end{remark}
...
\begin{remark}
	\label{th:VRSLH}
	$\VR$, $\SL$ are anonymous and neutral. They satisfy the Pareto property. Moreover, $\forall \prof$, $\VR(\prof) \subseteq H(\prof)$ and $\SL(\prof)\subseteq H(\prof)$.
\end{remark}
...
\begin{theorem}
  \label{th:subcorr}
   	$\FB, \VR, \SL \subset \PVe$.   
\end{theorem}
\begin{proof} \Cref{th:intersection}, together with \cref{th:FBH,th:VRSLH}, establishes that $\FB, \VR, \SL \subseteq \PVe$. To see the strictness of the inclusion, consider the profile
	\begin{equation}
...
\begin{remark}
 The proof of Theorem \ref{th:subcorr} shows that the union of $\FB, \VR, \SL$ is a proper subset of $\PVe$ as well.
\end{remark}
\commentOC{Can we integrate this remark into the statement of the theorem, by adding: “and $\FB \cup \VR \cup \SL \subset \PVe$”?} \commentRS{OK}
...
\end{definition}
\commentRS{To be coherent with our new terminology PEL-Compatible and PEL, I renamed EL as EL compatible and the stronger version as EL. In the proposition below, the set EL must be ELC. There were phrasing issues with EL compatibility, which I tried to fix. I also changed SPEL to PEL.}\commentMN{Thanks}
\commentOC{Then the EL in the following proposition should be changed to ELC, shouldn’t it?} \commentRS{Yes}
...
\commentRS{Shouldn't we call this MD compatible?}
\commentOC{I agree that “MD compatibility” is more coherent, but it is not a nice name (and impractical to use). I favor sticking to MD as we do not need the stronger one, but I do not object strongly if you prefer it otherwise.} \commentRS{I prefer coherence which simplifies the reading of the paper but let you to decide}
...
\commentRS{I propose to put Theorem 4 right after and Lemmata 2, 3, 4, 5 later, if possible to an appendix.}\commentMN{I agree}
\commentOC{We refer to these lemmas in several proofs of theorems that will not go to the appendix. Is this a problem?} \commentRS{To some extent, yes. However, I think putting the theorem before the lemmata simplifies the reading considerably.}
\begin{lemma}
	\label{th:PNonEmpty}
...
\end{remark}
\commentRS{I propose to delete the theorem below but call Propositions 5, 6, 7, 8, 9 theorems.}\commentMN{I agree with deleting the theorem. However, I suggest that Propositions 5-9 remain propositions to avoid having too many theorems}
\commentOC{I had in mind that we agreed to use what is now in a comment at the start of this article as the “selling argument” for the article. Thus, that we stick to five theorems that should correspond to the story that the selling argument describes. Do we want to adopt another approach instead?} \commentRS{Sorry, I didn't mean to change our approach. I let you to decide.}
...
\end{proposition}
\commentRS{Are we sure that the statement of the proposition is true. We can take k greater than half and the left side will still hold. Shouldn't we say k is at least $\khalf$?}\commentMN{Olivier may be better than me in this one.}
\commentOC{You are right. Also, perhaps the statement “$\forall P, \min_{\PE} (d \circ \lprof) ≤ k$” (for the LHS) is simpler?}
\begin{proof}
	\commentOC{TODO}
	If $k = \khalf$, it follows from \cref{th:BKkhalf} that $\forall \prof, \exists x \in \allalts \suchthat \max \lprof(x) ≤ k$, showing the "if" part. 
