...
\commentOC{It currently looks to me like the quantifier does not apply to the part after the textual “and” as it interrupts the mathematical statement, and I find it easier to parse with the logical and (so that the mathematical flow in the brackets is not interrupted), but I won’t insist if you don’t like it.}
...
Define the \emph{resolute RMD} rule as follows. It assigns to each Paretian alternative $a \in \PD$ \color{red} What is PD? Is it PE? \color{black}in a profile $\prof$ having losses $\lprof(a)$ the score (or “badness”) $s(a) = md(\lprof(a)) + \min\lprof(a) + m^2 \ind_{a \notin H(\prof)}$, where $m = \card{\allalts}$ and $d(l) = \max l - \min l$ as previously. The rule picks as winners the Paretian alternatives with minimal score, thus $f(\prof) = \argmin_{a \in \PD} s(a)$.
