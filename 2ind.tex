...
\end{proposition}
\begin{proof}
	Consider some profile $\prof$ with $x \in PE(\prof)$ and $\lprof(x) = (k, k)$ for some $k \in \intvl{0, m - 1}$. Let us show that $FB(\prof) = \set{x}$.
	Observe that for any $y \in FB(\prof)$, $\max \lprof(y) ≥ \max \lprof(x) = k$ (as FB picks minimal worst loss alternatives). Thus, $y$ (weakly) Pareto-dominates $x$. Since $x$ is Pareto-efficient, $x = y$.
\end{proof}
