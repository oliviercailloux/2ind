...
We say that such alternatives receive unanimous support at level $k$. Let $\rhoP = \min \set{k \in \losses \suchthat U(\prof, k) ≠ \emptyset} = \min_{x \in \allalts} \set{\max \lprof(x)}$ be the least loss level at which some alternative receives unanimous support.

\section{SCRs in the literature}

We start by defining the SCRs of the literature that we analyze in the paper.

\textbf{Unanimity compromise} is the SCR $\FB$ that picks all alternatives with unanimous support at $\rhoP$: $\FB(\prof) = U(\prof, \rhoP)$. 

\begin{remark}
	$\FB$ is anonymous and neutral. It satisfies the Pareto property. Moreover, $\FB(\prof) \subseteq H(\prof) \: \forall \prof$. The final observation comes from the fact that $\rhoP\leq \khalf$ for any profile $\prof$, a consequence of the following proposition that is a restatement of \citet[Theorem 1]{BramsKilgour2001}. We include a proof for completeness.
\end{remark}
...
\begin{remark}
	$\VR$, $\SL$ are anonymous and neutral. They satisfy the Pareto property. Moreover, $\VR(\prof),\SL(\prof)\subseteq H(\prof) \: \forall \prof$.
\end{remark}
...
We now explore the relationship between $\FB$, $\VR$ and $\SL$ and show that they can pick disjoint winners.
\begin{proposition}\label{th:different}
There is some $\prof$ such that $\FB(\prof) \cap \VR(\prof) = \emptyset, \FB(\prof) \cap \SL(\prof) = \emptyset$ and $\VR(\prof) \cap \SL(\prof) = \emptyset$.
\end{proposition}
\begin{proof}
...
\end{proof}
A particular member of the class of Pareto-and-veto rules is of special interest regarding its relation to $\FB$, $\VR$ and $\SL$ as well as to a property that will be introduced in the next section. We define the rule $\PVe = \PVv[\left(\floor{\frac{m - 1}{2}}, \floor{\frac{m - 1}{2}}\right)]$ as the rule in $\PVcl$ that gives the highest equal veto power to both individuals. Thus, under $PV^=$, we have $v_1=v_2= \floor{\frac{m - 1}{2}}$, implying $v_1=v_2=\frac{m-1}{2}$ when $m$ is odd and $v_1=v_2= \frac{m}{2}-1$ when $m$ is even. Note that $PV^=\in\PVbcl$ iff $m$ is odd.
\begin{theorem}
  \label{th:subcorr}
   	$\FB, \VR, \SL \subset \PVe$.   
\end{theorem}
\begin{proof} Observe that $\PVe(\prof)=H(\prof)\cap PE(\prof)$ for each $\prof$. xxxx


$UC$ picks all alternatives with unanimous support $\rho_{\prof}$: \cref{th:BKkhalf} implies that  $\rho_{\prof}\geq \floor{\frac{m}{2}}$, which ensures that any alternative selected by $UC$
is Pareto efficient and in $H(P)$, as desired. 

Likewise, $\VR$ selects only the Borda winners which are not among the worst  $\floor{\frac{m - 1}{2}}$ alternatives; each such winner thus belongs to $\PVe(\prof)$.

Finally, $\SL$ picks the best alternative of individual $1$ that is not among the worst $\floor{\frac{m - 1}{2}}$ alternatives of individual $2$ and the best alternative of $2$ that is not among the worst $\floor{\frac{m - 1}{2}}$ alternatives of $1$. Again, by construction, both alternatives are Pareto efficient and in the best-half.

To see the strictness of the inclusion, consider the profile
	\begin{equation}
...
\begin{remark}
 The proof of Theorem xxxx shows that the union of $\FB, \VR, \SL$ is a proper subset of $\PVe$ as well.   
\end{remark}
...
Note that \cref{th:inFH}, \cref{th:subPVe} and \cref{th:different} together provide a different proof of \cref{th:subcorr}.

\cref{th:knownInMr} results from \cref{th:subPVe,th:inFH,th:pvmr}.
\begin{theorem}
	\label{th:knownInMr}
