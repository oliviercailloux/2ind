...
\commentOC{I added an equivalent statement about UC so as to use it in the proof later (see my related comment later).}
...
\commentOC{I have now changed it throughout. However, it is not as nice as I thought: the complement that matters is the one to $m - 1$, not to $m$, so that the complement of $\khalf$ (to $m - 1$, thus) is $\floor{\frac{m - 1}{2}}$, or $\ceil{\frac{m}{2}} - 1$, both being not much intuitive; whereas it is more immediate to see that the complement of $\khalfAlt$ to $m - 1$ is $\floor{\frac{m - 1}{2}}$. However, I considered that the advantages of $\khalf$ over $\khalfAlt$ generally compensates for this nuisance.}
\commentRS{I am OK with all the comments until here. They can be deleted whenever the necessary action is taken.} 
\commentOC{Okay, I wait for Matías’ approval.}
...
\begin{definition}[$k$-Rawlsianism] 
	...
	$\forall \prof \in \allprofs,  x, y \in \allalts, \max \lprof(x) ≤ k \land \max \lprof(y) > k ⇒ y \notin f(\prof)$.
\end{definition}

$k$-Rawlsianism is binding at exactly those profiles where some alternative $x$ has its losses in $\intvl{0, k}^2$ (equivalently, satisfies $\max \lprof(x) ≤ k$), and is void at the other profiles. It is thus natural to wonder which values of $k$ make the constraint binding at every profile, so as to guarantee a minimal possible loss to every individual  whatever the profile.
...
\commentOC{We said previously that it would be nice to have a short proof for completeness (it would be a bit sad that the reader needs to refer to their article just for these starting results whereas the rest of our paper is entirely self-contained). I had given up at that time because I thought it would be difficult to prove. But thinking about it again (and borrowing from one of my proofs for our other paper in preparation about compromise rules), I realize that the proof is quite short. So I suggest to include it.}
\commentRS{OK. I deleted above the qualification "short" for the proof and made stylistic changes in the proof below.} 
\begin{theorem}
	\label{th:BKkhalf}
...
\commentOC{Agreed with Matías: the remark is about an alternative definition of $k$-Rawlsianism, I see no reason to move it much further away.}
\commentRS{Let's first agree on the content of the remark. What do we wish to say here? To my understanding, to point of Miguel is the following: using the "natural" half instead of our "artificial" one doesn't matter under SRMD (while it matters in other instances). Am I right, Olivier?} 
\commentMN{I agree with Remzi's interpretation, even though I did not talk to Miguel}
\commentOC{1. The content of the remark. We wish (rather, I wish) to talk about a variant of $k$-Rawlsianism that the reader may have in mind (or may simply find interesting to think about), to compare it with our definition and convince the reader that our definition is more interesting than the variant but that if she really prefers the variant, it’s all right, as they coincide for our choice of $k$.
2. The point of Miguel. He made several points. I agree that one of his points was about SRMD. And that has to be talked about after defining SRMD. But it’s not an argument for moving \cref{rk:variant}. Or I am missing something.}
\commentRS{There are two issues. ISSUE 1: For any k, remarking the difference between Rawlsianism and strict Rawlsianism. This is what Remark 4 says. This remark is unrelated to SRMD and I agree that we can make it here, if it is worth making. However, the paper is already full of concepts and I don't see the utility of introducing a new one which we never use. ISSUE 2: Choosing k as the "true" half is equivalent to our "artificial" half under SRMD. I find this worthy to remark and its place is, naturally, in the section where we define SRMD.} 
\commentOC{I find \cref{rk:variant} possibly enlightening but I admit that this may be overwhelming for the first reading. So if Remzi is opposed and Matías is not in favor of keeping it, we can delete it. Another possibility is to transform it into a footnote to make it clear that it can be skipped on first read (though I think that the clear demarcation as a Remark conveys the “can-be-skipped” information equally well).} 
\commentRS{Exactly, my subjective argument is that the cost of overwhelming exceeds the benefit of enlightenment. Let's see what Matias will say.} 
\commentMN{Thanks to both of you for this thorough analysis. Making it a footnote seems to me a good compromise. Is it ok?}
\commentOC{Ok for me.} \commentRS{Ok for me as well.}
...
%	By construction thus, $\forall a ≠ y \in A: \lprof(y)_{\ibar} < \lprof(a)_{\ibar}$, hence, $\lprof(y)_{\ibar} ≤ m - 1 - \card{(A \setminus \set{y})} = m - 1 - t$.
	...
	It follows that $\lprof(x)_{\ibar} < \lprof(y)_{\ibar}$, otherwise $y$ Pareto-dominates $x$; whence $\lprof(y)_i < \lprof(y)_{\ibar}$ otherwise $\lprof(x)_{\ibar} < \lprof(y)_{\ibar} ≤ \lprof(y)_i < \lprof(x)_i$ and $d(\lprof(y)) < d(\lprof(x))$, contradicting $x \in \argmin_{\PE} (d \circ \lprof)$.
	 ...
	 Observe that as $t \in \N$, $t ≥ \frac{2m - 4}{3} ⇒ t ≥ \ceil{\frac{2m - 4}{3}}$, and when $m ≥ 3$, $\ceil{\frac{2m - 4}{3}} ≥ \khalfAlt$; whence $t ≥ \khalfAlt$.%
	 \footnote{When $m$ is odd, $m ≥ 5 ⇒ \frac{2m - 4}{3} ≥ \frac{m - 1}{2} = \khalfAlt$ and when $m$ is even, $m ≥ 8 ⇒ \frac{2m - 4}{3} ≥ \frac{m}{2} = \khalfAlt$; when $m = 3$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{2}{3}} = 1 = \khalfAlt$; when $m = 4$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{4}{3}} = 2 = \khalfAlt$; when $m = 6$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{8}{3}} = 3 = \khalfAlt$.} 
	
	By construction, $\forall a ≠ y \in A: \lprof(y)_{\ibar} < \lprof(a)_{\ibar}$, hence, $\lprof(y)_{\ibar} ≤ m - 1 - \card{(A \setminus \set{y})} = m - 1 - t$.
	Also, $t ≥ \khalfAlt$ thus $m - 1 - t ≤ m - 1 - \khalfAlt = \floor{\frac{m - 1}{2}} ≤ t$.
...
%	\commentOC{It would be more elegant to show that any other alternative than $y$ has either $\max\lprof(x) ≤ t$ or $d(x) ≥ d(y)$.}
	...
	\commentOC{There seems to be a problem here: we don’t have that $t ≥ \khalf$. I don’t remember what I had in mind when writing this. Do you see a way of saving this argument?} \commentRS{OMG.}
...
\commentOC{I agree that Miguel suggested to compute that bound, but he didn’t say how much it would be (I suggested one third of $m$ but it was only a rough guess). Why Theorem 15? I don’t see the link with the veto levels. Isn’t the bound $\khalf$, as Matías writes? Consider $(abcdefgh)$ and $(efghabcd)$ for the other individual.} \commentRS{Right, my pointing to Theorem 15 is senseless. As Matias says the dispersion is at most $m/2$ and as your example shows it cannot be guaranteed to be less than $m/2$. Thus, $m/2$ is the maxmin value for dispersion. Miguel suggested to compute this value and proposed the condition to eliminate alternatives whose dispersion exceeds that value. However, as this value is $m/2$, the proposition of Miguel boils down to the first half condition, no? If so, this is an interesting remark to make at the end of this section.}\commentMN{Very smart example Olivier. I am fine with adding a small result. However, I am now a bit confused to write such a result. In your example you have an even number of alternatives. You get that the minimal dispersion is exactly $m/2$. With an odd number of alternatives can one obtain the same example and show that the minimal level is $\khalf$? }
...
\color{green} NEW VERSION:More resoluteness can be achieved by adding a requirement that among the Pareto efficient alternatives in the FH. Define to that effect the \emph{resolute RMD} rule as follows. This rule makes a selection of the strong RMD alternatives, by picking the one(s) with the highest sum of losses: thus $f(\prof) = \argmax_{a \in SMRD(\prof)} \lprof(a)_1+\lprof(a)_2 $. \commentRS{We mean the lowest sum of losses, no?}
...
\commentRS{A possibility is the define the rule as in the old version and characterize it with the lowest sum of losses condition.}
...
\commentRS{We should add to the conclusion our findings about the strong RMD rule. We should compare it to the VR rule. Should we mention the rule proposed by Hillas?}
\commentMN{Yes, The Hillas rule is a good idea.}
