...
We now discuss the relationship of the class $\PVcl$ to the $\FHcl$ property. To this end, we define $PV^=$ as the Pareto-and-veto rule that gives the highest equal veto power to both individuals. Thus, under PV^=$, we have v_1=v_2= \floor{\frac{m - 1}{2}}$, implying v_1=v_2=\frac{(m-1)}{2}$ when $m$ is odd and v_1=v_2= $\frac{m}{2}-1$ when $m$ is even. Note that when $m$ is odd PV^=$\in\PVbcl$ while PV^=$\in\PVcl\minusPVbcl$ when $m$ is even.  \commentRS{the m even part must be corrected as PV minus PVb} 
...
\end{proof}
The following remarks is a consequence of Proposition 1. \commentRS{I am sure there is a way to refer to Proposition 1 without writing it explicitly} 
\begin{remark}
...
\end{proof}
 \commentRS{There is a point I want to discuss. It will be quicker face to face} 
