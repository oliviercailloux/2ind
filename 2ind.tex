...
\commentOC{I add the corollary here below just to have an idea of what it looks like, then we can decide.}

\begin{corollary}
	For any $m ≥ 3$, a rule $\PVv[(v_1, v_2)] \in \PVbcl$ satisfies PEL iff $v_i = \floor{\frac{m - 1}{2}}$ and $v_{\ibar} = \ceil{\frac{m - 1}{2}}$ for $\set{i, \ibar} = \set{1, 2}$.
\end{corollary}
