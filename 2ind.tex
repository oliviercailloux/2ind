...
Given two SCRs $f$, $f'$, we write $f \subset f'$ to indicate that $f$ is a \commentMN{proper subcorrespondence, we did not define correspondence, how to do it? Maybe $f$ is a selection of $f'$?} of $f'$.
\commentOC{Here is a proposal. And I suggest to write $\subsetneq$ instead of $\subset$ as in some sub-disciplines of mathematics, $\subset$ is the well-established symbol for “subset-in-the-large-sense”, AFAIU.
If this is adopted, we have to change everywhere accordingly (that is, in one place).
Also, I have uniformized to the version with an hyphen.}
\commentOC{Viewing such functions as relations on $\allprofs × \allalts$, we can write $f \subseteq f'$ to denote a rule that “refines” $f'$, in the sense that $\forall \prof \in \allprofs: f(\prof) \subseteq f'(\prof)$. When $f \subseteq f'$ and $f ≠ f'$, we write $f \subsetneq f'$ and say that $f$ is a sub-correspondance of $f'$.}
...
\commentMN{Do we need this? It looks a bit redundant Given $v_i \in \intvl{0, m - 1}$, define $a_i = m - v_i - 1 \in \intvl{0, m - 1}$ as the highest acceptable loss level for individual $i$.} 
\commentOC{I find it slightly helpful.}
...
\commentRS{I suggest to call the proposition below a theorem and the theorem that follows it a proposition.} \commentMN{What about a single theorem that includes both results? It makes sense since both deal with $PV^=$} \commentRS{Still the two results are of different nature, I think.}
\commentOC{I stick to my guns. We should first decide how we want to sell the paper; and we still didn’t find a better approach than the five-points proposal that we used to have, despite its shortcomings. I let you find a better plan as you didn’t like it.}

\commentMN{Actually, I find confusing Theorem 2, isn'it more or less equivalent to Proposition 2?? } \commentRS{Yes but the two results are in terms of different concepts.} \commentMN{I did not have strong views on that, let's leave it as it is.}
...
\commentOC{I find this lemmas-and-proofs-embedded-in-a-bigger-proof approach a bit hard to parse, and I have never seen this anywhere. I suggest to drop the outer proof environment and instead write here “proof of theorem X”.}
...
\commentMN{In my opinion, Theorem 5 and Prop 9 can be removed. They do not add much to the section.}
\commentOC{I do not object, considering that the majority is not on my side. I let you find a strategy for naming and presenting our results.}
