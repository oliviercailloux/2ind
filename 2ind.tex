...
\begin{theorem}
	...
	\commentOC{That’s incorrect, isn’t it? We mean: $[\MRcl \cap \MDcl ≠ \emptyset]$ if and only if $m ≤ 6$ or $m = 8$.}\commentRS{Perhaps we can state the theorem as: "There exists a SCR f in..."}
\end{theorem}
\begin{proof}
...
\begin{remark}
	...
	\begin{equation}
		...
		\end{array}.
	\end{equation}
	Observe that $\FB(\prof) = \set{y}$ while the $\MD$ principle mandates picking at least $x$.
\end{remark}
...
\end{definition}
\commentOC{Do we mean $f(\prof) \cap \argmin_{H(\prof) \cap \PE} (d \circ \lprof) ≠ \emptyset$, $\forall \prof \in \allprofs$? Otherwise this seems incompatible with PE.} \commentRS{Yes, we mean that. I don't understand your two comments that follow.} \commentOC{Why incompatible with PE? }
\commentOC{I didn’t write the question in the previous comment.}
...
Thus, by \cref{th:pvmr}, among the rules in $\PVcl$, $\PVe$ is the only candidate to satisfy RMD. 
\commentOC{I don’t see how \cref{th:pvmr} applies here. Do we suggest that RMD is stronger than MR? That doesn’t seem correct to me.}\commentRS{By that theorem, there are only three PV rules that are MR. Our examples in the proof eliminate two of them and the equal veto remains as the only candidate.}
\commentOC{What’s the link with RMD?}\commentRS{It may be good to clarify the relation of RMD to MR and MD. MD and RMD are logically independent. However, RMD implies MR. AM I wrong?}\commentMN{I think the section is good enough. It only suggests that out of the rules we considered only $PV^=$$ passes the RMD test. I would leave it like this.}
As $\PVe$ chooses all Pareto efficient alternatives in $H(\prof)$, it always picks the alternative that minimizes the loss dispersion.
\end{proof}
