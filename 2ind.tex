...
%	But any rule that satisfies $MR$ may not include the alternative $a_{k}$ as a winner, as it is not in the first half, hence those rules fail $MD$.
...
\commentOC{Final part argument wording should be improved.}
\end{proof}
\commentRS{I think we don't need Theorem 4 below and the related material} \commentMN{I agree to discard this material}
...
\begin{theorem}
	$\forall t \in \intvl{\khalf, m - 1}: m < \frac{3t + 5}{2} ⇒ [\forall \prof: \lprofinv(\intvl{0, t} × \intvl{0, t}) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset]$. (Equivalently: $t > \frac{2m - 5}{3}$ or $t ≥ \frac{2m - 4}{3}$.)
	\commentOC{Check: with $t = 1$, $m = 3$ we have indeed $t > \frac{2m - 5}{3} = \frac{1}{3}$ or equivalently $t ≥ \frac{2m - 4}{3} = \frac{2}{3}$, and the conclusion holds.}
	%Problem Houston. The conclusion also holds with $t = 1$, $m = 3$ but where $t < \frac{2m - 4}{3} = \frac{2}{3}$ is false.}
\end{theorem}
\begin{proof}
	...
	%In other words, $\lprof(x)(1, 2) = \set{k, m - 1 - k - r}$, with $k ≥ m - 1 - k - r$. 
	It follows that $d(\lprof(x)) ≥ 2k + 1 - m$.
	
	Let $A = \set{a \suchthat \lprof(a)_i ≤ t}$ designate the $t + 1$ top alternatives for $i$. 
	
	If $k ≤ t$, the proof is done, so assume that $k > t$. 
	
	Let $B = \set{b \suchthat t < \lprof(b)_i < k}$ designate the $k$ top alternatives for $i$ except for those in $A$.
 
	Observe that $\forall b \in B: \lprof(b)_{\ibar} > \lprof(x)_{\ibar}$ as otherwise $b$ Pareto dominates $x$. 
	
	Observe that $\forall b \in B: \lprof(b)_{\ibar} > \lprof(b)_i$, for the following reason.
	We know that $\lprof(b)_i < \lprof(x)_i = k$ by definition of $B$.
	Now note that $\lprof(x)_{\ibar} < \lprof(b)_{\ibar} ≤ \lprof(b)_i < \lprof(x)_i$ is impossible as then $d(b) < d(x)$. \commentOC{Unless $b$ is PD.} So, it must be that $\lprof(b)_{\ibar} > \lprof(b)_i$.
	
	Pick $y = \argmin_A \lprof(.)_{\ibar}$ as the best alternative for voter $\ibar$ among $A$. By construction thus, $\forall a ≠ y \in A: \lprof(y)_{\ibar} < \lprof(a)_{\ibar}$, hence, $\lprof(y)_{\ibar} ≤ m - 1 - \card{(A \setminus \set{y})} = m - 1 - t$.
	
	We have $t ≥ \khalf$ thus $m - t ≤ m - \khalf = (m - 1) - \khalf + 1 = \floor{\frac{m - 1}{2}} + 1$.
	
	Consider any $b \in B$. We established already that $\lprof(b)_{\ibar} > \lprof(b)_i$, and by definition of $B$, $\lprof(b)_i ≥ t + 1$. It follows that $\lprof(b)_{\ibar} > t + 1 ≥ \floor{\frac{m - 1}{2}} + 1 ≥ m - t ≥ \lprof(y)_{\ibar}$. We conclude that $\lprof(b)_{\ibar} > \lprof(y)_{\ibar}$.
	Therefore, $\lprof(y)_{\ibar} ≤ m - 1 - \card{(A \setminus \set{y})} - \card{B} = m - 1 - \card{(A \cup B \setminus \set{y})} = m - 1 - (k - 1) = m - k$.
	
	Note also that $\lprof(x)_{\ibar} < \lprof(y)_{\ibar}$, otherwise $y$ Pareto-dominates $x$; whence $\lprof(y)_i < \lprof(y)_{\ibar}$ otherwise $\lprof(x)_{\ibar} < \lprof(y)_{\ibar} ≤ \lprof(y)_i < \lprof(x)_i$ and $d(y) < d(x)$ \commentOC{Unless $y$ is PD.}
	It follows that $d(y) ≤ m - k$.
%	\commentOC{Problem here, could have $\lprof(y)_i = t$ and $\lprof(y)_{\ibar} = 0$, hence $d(y) = t$, and when $k ≥ t + 1$, thus $- k ≤ - t - 1$, we have $m - k ≤ m - t - 1 ≤ t$, hence $d(y) ≥ m - k$. And $t > m - k$ for example with $m = 9, t = 6, k = 7$.}
	%Note that the other alternatives in $A$ have non lower dispersion than $y$. 
	
	When $m - k ≤ 2k + 1 - m$ for all $k > t$, we have that $\forall P: d(\lprof(y)) ≤ d(\lprof(x))$ (equiv, $y$ is in the set). Equivalently, iff $2m ≤ 3k + 1$ for $k = t + 1$, equivalently, $2m ≤ 3t + 4$. 
\end{proof}

\begin{proof}[Simplification attempt]
	If $t + 1 ≥ \frac{2m - 1}{3}$ [equivalently, $2 (t + 1) - m + 1 ≥ m - t - 1$], then pick any $i \suchthat \exists x \in MD \suchthat \lprof(x)_i ≥ \lprof(x)_{\ibar}$; define $A = \set{a \suchthat \lprof(a)_i ≤ t}$; define $y = \argmin_A \lprof(.)_{\ibar}$; we claim that $\lprof(x)_i > t ⇒ y \in MD$.
	
	Observe that $y \in \PE$: for $i$, only the alternatives in $A$ may be better than $y$, and those are worst than $y$ for $\ibar$ by definition of $y$.
	
	Pick any $x \in MD \suchthat \lprof(x)_i ≥ \lprof(x)_{\ibar}$. Assume that $\lprof(x)_i > t$. 
	Therefore, $\lprof(y)_i < \lprof(x)_i$ (as $y \in A$).
	It follows that $\lprof(x)_{\ibar} < \lprof(y)_{\ibar}$, otherwise $y$ Pareto-dominates $x$; whence $\lprof(y)_i < \lprof(y)_{\ibar}$ otherwise $\lprof(x)_{\ibar} < \lprof(y)_{\ibar} ≤ \lprof(y)_i < \lprof(x)_i$ and $d(\lprof(y)) < d(\lprof(x))$, contradicting $x \in MD$.
	
	Note that $d(\lprof(y)) ≤ m - 1 - t$:  by construction, $\forall a ≠ y \in A: \lprof(y)_{\ibar} < \lprof(a)_{\ibar}$, hence, $\lprof(y)_{\ibar} ≤ m - 1 - \card{(A \setminus \set{y})} = m - 1 - t$; and, using $\lprof(y)_i < \lprof(y)_{\ibar}$, $d(\lprof(y)) ≤ m - 1 - t$. 
	
	And $m - 1 - t ≤ t$.
	\commentOC{Why?}
	
	To conclude, observe that $\min \lprof(x) ≤ m - 1 - \max \lprof(x)$: the alternative $x$ has $\max \lprof(x) = \lprof(x)_i$ alternatives better than it for the voter $i$, and therefore must have at least those $\max \lprof(x)$ alternatives worst than it for the other voter, otherwise it is Pareto-dominated by at least one of those $\max \lprof(x)$ alternatives. This yields $d(\lprof(x)) = \max \lprof(x) - \min\lprof(x) ≥ \max \lprof(x) - (m - 1 - \max \lprof(x)) = 2 \lprof(x)_i - m + 1 ≥ 2 (t + 1) - m + 1$, hence by our hypothesis, $d(\lprof(x)) ≥ m - t - 1 ≥ d(\lprof(y))$, which implies that $y \in MD$.
...
\begin{proof}[other direction!]
	Assume that $\khalf ≤ t ≤ \frac{2m - 5}{3}$ and let’s define $\prof$ such that the set is empty.
	\commentOC{For $m ≤ 7$, this only holds for $m = 7, t = 3$.}
	
	Define the sequences of alternatives $A = (a_1, …, a_{t + 1})$ and $C = (c_1, …, c_{m - t - 2})$. 
	We have $m = t + 1 + (m - t - 2) + 1$. 
...
\begin{remark}
For example, when $t = \khalf$, $m$ odd, we obtain $m ≥ \frac{3 (m - 1) + 10}{4}$ thus $m ≥ 7$. 
	And for $m$ even, $m ≥ \frac{3m + 10}{4}$ thus $m ≥ 10$.
	Conversely, we obtain a guaranteed non empty set when $m < \frac{3t + 5}{2}$, equivalently, $t > \frac{2m - 5}{3}$, thus, when we consider up to ranks at least $\ceil{\frac{2}{3}m}$.
\end{remark}
