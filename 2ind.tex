...
\begin{proof}
	...
	Pick $i \in \argmin l'$, equivalently, $i \in \argmax l$. Recall that $l(i) < l'(i)$; also, $\min l = l(\ibar) < \max l' = \max l$.
	Define the (possibly empty) sequences $A_1 = (a_1, …, a_{l_{\ibar}})$, $A_2 = (a_{l_{\ibar} + 1}, …, a_{\max l' - 1})$, $B_1 = (b_1, …, b_{l_{\ibar}})$, $B_2 = (b_{l_{\ibar} + 1}, …, b_{\max l' - 1})$, $C = (c_1, …, c_{m - 2 \max l'})$, and the profile $\prof$ defined as:
	\begin{equation}
		\begin{array}{*{25}{l}}
	...
	Observe also that $\lprof(x)_i = l(i)$, $\lprof(x)_{\ibar} = l(\ibar)$, $\lprof(y)_i = l'(i)$ and $\lprof(y)_{\ibar} = \max l' = l'(\ibar)$, thus, $\lprof(x) = l$ and $\lprof(y) = l'$.
	
	Consider now the profile $\prof'$ associating to each individual the preference of the other individual in $\prof$, as follows:
	\begin{equation}
		\begin{array}{*{25}{l}}
			i &\mapsto & B_1 & x &B_2 & y & C & A_1 & A_2 \\
			\ibar &\mapsto & A_1 & y & A_2 & x & C & B_1 & B_2
		\end{array}.
	\end{equation}
