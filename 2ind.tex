...
Given $p, q \in \N$, let $\intvl{p, q} = [p, q] \cap \N $ denote the interval of integer numbers between $p$ and $q$. The loss of individual $i$ at alternative $x$ is  defined as the number of alternatives that the individual prefers to $x$: $\lprof(x)_i = \card{\set{y \in \allalts \suchthat y \prefi x}}$.
...
Given two loss vectors $l, l' \in \lvs$, we say that $l$ is strictly smaller than $l'$, $l < l'$, iff $\forall i: l_i ≤ l'_i \land l ≠ l'$. We also write $l ≤ l'$ to denote that $l$ is weakly smaller than $l'$, meaning, strictly smaller or equal. Let \color{red}$\min_N \lprof(x) = \min_{i \in N} \lprof(x)_i \in \N$ This notation can be improved, what about $\lprof^{min}(x)$?? \color{black} and $\sum_N \lprof(x) = \sum_{i \in N} \lprof(x)_i$ denote, respectively, the minimal loss and the sum of the losses associated to $\lprof(x)$.


\color{red}I changed the previous paragraph, please check \color{black}
...
Formally, given a profile $\prof \in \allprofs$ and an individual $i$, let $H^i(\prof) = \{x \in \allalts \suchthat \lprof_i(x) ≤ \ceil{\frac{m - 1}{2}}\} $ denote the set of alternatives in the best half of player $i$'s preference. 
...
Given $\prof$ and a loss level $k \in \losses$, define $U(\prof, k) = \set{x \in \allalts \suchthat \lprof(x) ≤ (k, k)}$ as the set of alternatives imposing losses not higher than $k$ for all individuals. We say that such alternatives receive unanimous support at level $k$. Let $\rhoP = \min \set{k \in \losses \suchthat U(\prof, k) ≠ \emptyset}$ be the least loss level at which some alternative receives unanimous support. By Theorem 1 of \cite{BramsKilgour2001} we have $\forall \prof \in \allprofs$, $\rhoP ≤ \ceil{\frac{m - 1}{2}}$.
...
The remark follows from the definition of the different rules since each of these rules selects only alternatives among the top-half alternatives of both individuals. 
...
\commentOC{I think that this should be called “small dispersion”.}
...
\commentOC{Isn’t it interesting to keep the definitions that generalize the loss related concepts to other spread measures than dispersion? (As we did previously.)}
...
For any $k\in\{1,\ldots,m\}$, we say that the SCR $f$ satisfies the minimal dispersion principle $MD(k)$ iff $\forall \mathbf{P}$, $\forall x,y \in PE(\mathbf{P})$, 
if $(d \circ \lprof)(x)-(d \circ \lprof)(y)\geq k$, then $x\not \in f(\mathbf{P})$.


Note that $MD(k)$ gets weaker with growing $k$. $MD(1)$ implies to pick among alternatives with a minimal spread. $MD(m-1)$ only requires that an alternative with extreme spread (i.e., ranked first by a voter and last by another) should not be elected in the presence of a Pareto efficient alternative that is ranked at the same level by both voters. $MD(m)$ is vacuous. Note also that $MD(k)$ implies PEL iff $k=1$. We refer to $MD(1)$ as strong PEL (SPEL).
...
This shows an incompatibility between the minimal Rawlsian and minimal dispersion principles. As a specific instance, SPEL is incompatible with the minimal Rawlsian principle. We had already see that PEL is failed by VR, SL and all rules in PV while satisfied by FB which, by the result above, fails SPEL.


QUESTIONS:

- Can we identify an interesting SCR that satisfies SPEL?
- What interesting refinements VR, SL or FB have? How to use SPEL to refine them? I give a few examples that illustrates the point of the question. For example, at the profile 
\begin{center}
	$
	\begin{array}{cccccc}
