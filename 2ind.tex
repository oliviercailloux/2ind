...
A SCR $f$ is anonymous iff, for any $({\prefi[1]}, {\prefi[2]}) \in \allprofs$, $f({\prefi[1]}, {\prefi[2]}) = f({\prefi[2]}, {\prefi[1]})$.
A SCR $f$ is neutral iff for any permutation $\sigma$ over $\allalts$ and any profile $\prof \in \allprofs$, $\sigma \circ f(\prof) = f(\sigma \circ \prof)$.

Given $p, q \in \R$, let $\intvl{p, q} = [p, q] \cap \N $ denote the interval of integers between $p$ and $q$. The loss of individual $i$ at $\prof$ for alternative $x$ equals the number of alternatives that $i$ prefers to $x$: $\lprof(x)_i = \card{\set{y \in \allalts \suchthat y \prefi x}}$. The loss vector of $x$ at $\prof$ is denoted $\lprof(x)=(\lprof(x)_1, \lprof(x)_2) \in \lvs$. Given two loss vectors $l, l' \in \lvs$, $l$ is smaller than $l'$, $l ≤ l'$, iff $l_i ≤ l'_i$ $\forall i$. We also write $l < l'$ when $l$ is strictly smaller than $l'$, meaning, smaller and different. 
...
In the \textbf{veto-rank rule} $\VR$, each individual vetoes her worst $\floor{\frac{m - 1}{2}}$ alternatives, and the Borda winners among the non vetoed alternatives are picked: $\VR(\prof) = \argmin_{H(\prof)} \sum \lprof = \set{x \in H(\prof) \suchthat \forall y \in H(\prof): \sum \lprof(x) ≤ \sum \lprof(y)}$.

In the \textbf{shortlisting rule} $\SL$, the best alternative of individual $1$  that is not among the worst $\floor{\frac{m - 1}{2}}$ alternatives of individual $2$  and the best alternative of $2$ that is not among the worst $\floor{\frac{m - 1}{2}}$ alternatives of $1$ are selected: $SL(\prof) = \cup_{i \in N} (\argmin_{x \in H^i(\prof)} \lprof(x)_{\ibar})$.
...
\commentRS{I moved the remark above here where it is better placed, I think.}
...
\commentMN{Do we need this? It looks a bit redundant Given $v_i \in \intvl{0, m - 1}$, define $a_i = m - v_i - 1 \in \intvl{0, m - 1}$ as the highest acceptable loss level for individual $i$.} For $v=(v_1,v_2)$, the rule $\PVv(\prof) = \cap_{i \in N}\set{x \in \allalts \suchthat \lprof(x)_i ≤ a_i} \cap \PE$ picks all Pareto-efficient alternatives that no individual vetoes. 
...
\commentRS{I suggest to call the proposition below a theorem and the theorem that follows it a proposition.} \commentMN{What about a single theorem that includes both results? It makes sense since both deal with $PV^=$}
...
\commentRS{A verbal explanation ıf each lemma may help.}\commentMN{Done} \commentRS{OK. I made minor cosmetic changes.}
\begin{theorem}
	\label{th:noMRMD}
...
\commentRS{I see your point but given the propositions of this section, stating the theorem below seems like an unnecessary repetition. One way can be to state the theorem at the very beginning but this risks to be overwhelming to the reader. }
...
\commentOC{Shall we prove the last claim? The proof is that, observing that $\sum l = \min l + \min l + d(l)$ and $\sum l = \max l + \max l - d(l)$, when $d(l) = d(l')$, the statement $\sum l < \sum l'$ is equivalent both to $\min l < \min l'$ and to $\max l < \max l'$.}\commentMN{Seems fine to me}
...
\commentOC{In the table, we mean PEL instead of SPEL, right?}\commentMN{I am confused, not sure which one is the correct one}
