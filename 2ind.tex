...
\begin{lemma}
	\label{th:caractEmptyL}
	Given $m ≥ 3$, the following hold:
	\begin{enumerate}
		\item \label{it:Pnonem} $\forall \prof: \lprofinv(\intvl{0, \ceil{\frac{2m - 4}{3}}} × \intvl{0, \ceil{\frac{2m - 4}{3}}}) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset$,
		\item \label{it:Pem2} $\exists \prof \suchthat \lprofinv(\intvl{0, \ceil{\frac{2m - 7}{3}}} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$.
	\end{enumerate}
\end{lemma}
...
\end{theorem}
\begin{proof}
	We in fact prove a slightly stronger claim, namely the equivalence of the two statements in the theorem and the following two:
	...
	For the claim $\ref{it:tbound} ⇒ \ref{it:Pt}$, consider any $t ≥ \frac{2m - 4}{3}$ and any $\prof \in \allprofs$; let us show that the set $\lprofinv(\intvl{0, t} × \intvl{0, t}) \cap \argmin_{\PE} (d \circ \lprof)$ is non-empty. Consider any $x \in \argmin_{\PE} (d \circ \lprof)$, and let $i$ denote any individual such that $\max \lprof(x) = \lprof(x)_i$. 
	...
	It follows that $\lprof(x)_{\ibar}<\lprof(y)_{\ibar}  $ to avoid that $y$ Pareto-dominates $x$. Moreover, if $\lprof(y)_{\ibar} \leq \lprof(y)_i $, then \[\lprof(x)_{\ibar} < \lprof(y)_{\ibar} ≤ \lprof(y)_i < \lprof(x)_i\] which implies that $d(\lprof(y)) < d(\lprof(x))$, contradicting $x \in \argmin_{\PE} (d \circ \lprof)$. Thus, we have $\lprof(y)_i < \lprof(y)_{\ibar}$.
 	 ...
 	 Observe that as $t \in \N$, $t ≥ \frac{2m - 4}{3} ⇒ t ≥ \ceil{\frac{2m - 4}{3}}$, and when $m ≥ 3$, $\ceil{\frac{2m - 4}{3}} ≥ \khalfAlt$.
	Indeed, when $m$ is odd, $m ≥ 5 ⇒ \frac{2m - 4}{3} ≥ \frac{m - 1}{2} = \khalfAlt$ and when $m$ is even, $m ≥ 8 ⇒ \frac{2m - 4}{3} ≥ \frac{m}{2} = \khalfAlt$; when $m = 3$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{2}{3}} = 1 = \khalfAlt$; when $m = 4$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{4}{3}} = 2 = \khalfAlt$ and when $m = 6$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{8}{3}} = 3 = \khalfAlt$. 
	We obtain that $t ≥ \khalfAlt$.
	...
	By \cref{th:sumlosses}, $\min \lprof(x) ≤ m - 1 - \max \lprof(x)$, whence $d(\lprof(x)) = \max \lprof(x) - \min\lprof(x) ≥ \max \lprof(x) - (m - 1 - \max \lprof(x)) = 2 \lprof(x)_i - m + 1 ≥ 2 (t + 1) - m + 1$.
	
	To conclude, note that :
\[t ≥ \frac{2m - 4}{3} \Longleftrightarrow t + 1 ≥ \frac{2m - 1}{3} ⇔ 2 (t + 1) - m + 1 ≥ m - t - 1,\]
	 and hence $d(\lprof(x)) ≥ m - t - 1 ≥ d(\lprof(y))$, which implies that $y \in \argmin_{\PE} (d \circ \lprof)$.
	
	The claims $\ref{it:Pt} ⇒ \ref{it:Ptbig2}$ and $\ref{it:Pt} ⇒ \ref{it:Ptbig1}$ hold thanks to set inclusion.
	
	Turning now to the claim $\ref{it:Ptbig2} ⇒ \ref{it:tbound}$ (a similar claim shows that $\ref{it:Ptbig1} ⇒ \ref{it:tbound}$ by swapping the role of the individuals), consider some $t < \frac{2m - 4}{3}$ so that \ref{it:tbound} fails. Our aim is to prove that there is some profile $\prof$ such that $\lprofinv(\intvl{0, t} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$, violating \ref{it:Ptbig2}. 
	To this end, we define $t' = \max \set{\khalf, t}$
	and prove the stronger claim that $\lprofinv(\intvl{0, t'} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$. 
 
	Observe that $m = t' + 1 + (m - t' - 2) + 1$.  We label the alternatives as $\set{a_1, …, a_{t' + 1}, c_1, …, c_{m - t' - 2}, x}$ and consider the sequences $A = (a_1, …, a_{t' + 1})$ and $C = (c_1, …, c_{m - t' - 2})$. Define the preferences of individual $1$ (resp. individual 2) as $(A, x, C)$ (resp. $(C, x, A)$).
