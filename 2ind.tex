...
RMD strengthens MR (thus, Strong RMD as well). Also, Strong RMD (thus, RMD) is not stronger than MD, for large enough values of $m$, as a Paretian strict minimizer of dispersion might be out of the first half. The following theorem states the relationship between SRMD and MD more precisely.
...
\commentOC{We might think about writing something about $\RMDcl$ wrt $\MDcl$ as well, in the same spirit.}
\begin{theorem}
	$[m ≤ 6 \lor m = 8] ⇔ \SRMDcl \subseteq \MDcl ⇔ \SRMDcl \cap \MDcl ≠ \emptyset$.
\end{theorem}
\begin{proof}
	First consider $[m ≤ 6 \lor m = 8] ⇒ \SRMDcl \subseteq \MDcl$. The proof of \cref{th:noMRMD} indicates that $\forall \prof: H(P) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset ⇔ m ≤ 6 \lor m = 8$. Thus, when $m ≤ 6 \lor m = 8$, the alternatives that are paretian minimizers of dispersion in the first half (within which Strong RMD requires to pick) are also global paretian minimizers of dispersion. 
...
\commentOC{More resolute is better, IMHO, when (as here, it seems to me) there is a reasonable argument to break the tie.}
