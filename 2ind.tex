...
Théorèmes :
	\begin{itemize}
		\item relations entre règles connues (\cref{th:subcorr})
		\item quelles règles satisfont MR (\cref{th:equal,th:pvmr})
...
We say that such alternatives receive unanimous support at level $k$. 
Note that $\min_{x \in \allalts} \set{\max \lprof(x)}$, that we call the “minimal worst loss”, represents the least loss level at which some alternative receives unanimous support; formally: $\min_{x \in \allalts} \set{\max \lprof(x)} = \min \set{k \in \losses \suchthat U(\prof, k) ≠ \emptyset}$.
...
\textbf{Unanimity compromise} is the SCR $\FB$ that picks all alternatives with unanimous support at the minimal worst loss $\rhoP = \min_{x \in \allalts} \set{\max \lprof(x)}$; formally: $\FB(\prof) = U(\prof, \rhoP) = \argmin_{x \in \allalts} \set{\max \lprof(x)}$.
\commentOC{I added the part $ = \argmin_{x \in \allalts} \set{\max \lprof(x)}$.}
\begin{remark}
	\label{th:FBH}
...
\begin{theorem}
  \label{th:subcorr}
   	$\FB, \VR, \SL \subset \PVe$ and $\FB \cup \VR \cup \SL \subset \PVe$.   
\end{theorem}
...
\commentOC{You do have a point, but by lazyness I must say I’d rather leave it as it is if I am the one to change stuff and check thoroughfully that we didn’t miss one renaming. However I don’t object if anybody wants to change it! :-)}
...
\commentOC{I am not against the principle of this idea, but I currently fail to see how to implement it. Are you suggesting that we simply move these Lemmas to the appendix and change nothing else? Then here we will have Theorem 4 whose proof starts with referring to a Lemma that will not have been introduced anyhow, which looks very odd and will appear as a mistake to the reader, I suppose. Should we write before “The proofs below use several lemmas that are stated and proven in the appendix”? But isn’t it still odd even with that sentence that the proof starts with referring to a lemma that has not even been stated? Or should we just state the lemmas and leave the proofs in appendix?
Again, I don’t object in principle so please just implement your idea if you have something concrete in mind.}
\begin{lemma}
	\label{th:PNonEmpty}
...
\commentOC{I still like the idea of introducing our article as stating these five points as written in my introductory comment. 
Also, if we like it, then perhaps it’d better to have just five theorems (instead of six currently) to match these five points.}
...
\commentOC{(A stupid question but just to be sure) we mean PEL instead of SPEL, right?}
...
\commentOC{In the table, we mean PEL instead of SPEL, right?}
