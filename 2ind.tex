...
Within the spirit of equal loss, we propose the \textit{minimal dispersion principle} as another strengthening of the (weak) equal loss principle. The \textit{dispersion} of an alternative is the difference between the two ranks at which is it placed at the preferences of the two individuals. The minimal dispersion principle requires that an alternative whose dispersion is minimal must be among the outcomes. This principle turns out to be logically incompatible with the minimal Rawlsian principle, thus being failed by fallback bargaining. \commentRS{I propose to put the paragraph below as a footnote here.}
...
In concordance with the ceiling established by Theorem 1 of \cite{BramsKilgour2001}, we use the term “half” to mean the smallest integer $k$ that exceeds $m-k$ \commentOC{to \emph{refer to} the smallest integer $k$ that \emph{strictly} exceeds $m - k$?} and for every profile $\prof \in \allprofs$, we let $H(\prof) = \set{x \in \allalts \suchthat \lprof(x) ≤ (\ceil{\frac{m - 1}{2}}, \ceil{\frac{m - 1}{2}})}$ denote the set of alternatives reaching the best half of every individual’s preference. Given $\prof$ and a loss level $k \in \losses$, define $U(\prof, k) = \set{x \in \allalts \suchthat \lprof(x) ≤ (k, k)}$ as the set of alternatives imposing losses not higher than $k$ for all individuals. 
...
\commentRS{I tried to resolve the matter by revising the relevant paragraph at the intro. Can you have a look?}
\commentMN{I agree with the modification}
\commentOC{I had in mind to obtain the current definition of MR as a formal result rather than as a definition, something like the following (which unfortunately is too complex to be satisfactory).}
\begin{definition}[$k$-Bounded Rawlsianism] Given $k \in \intvl{0, m - 1}$, a SCR satisfies $k$-Bounded Rawlsianism iff it selects its winners among those alternatives whose losses are within $\intvl{0, k}$ for both voters. Formally:
	$\forall \prof \in \allprofs,  f(\prof) \subseteq \lprofinv(\intvl{0, k} × \intvl{0, k})$.
\end{definition}
\begin{definition}[Minimal Rawlsian (MR)]
...
\commentMN{Proof of Proposition 7 improved}
\commentRS{I proposed a further improvement to the proof of Proposition 7}
\commentOC{Beautifully short, but with a tiny bit left to the reader. I have extended it to cover this remaining bit, though it is now no more beautifully short. To be decided…}
...
Note that $d \in \Sigma$ coincides with multiple commonly used spread measures, as shown in \cref{sec:spreads} \commentOC{Appendix? Footnote? Remark? To be discussed}. 
...
\end{definition}
$MD$ requires the outcome to contain Paretian alternatives whose loss vectors have minimal dispersion \commentOC{I’d write \emph{some} Paretian alternatives}. As such, $MD$ is another strengthening of $PEL$ while it is logically independent of $SPEL$. Nevertheless, although there exists rules that satisfy both $SPEL$ and $MR$, $MD$ turns out to be logically incompatible with $MR$.
\begin{theorem}
	When $m ≥ 7$ is odd, there is no $f \in \mathcal{MR}$ that satisfies MD.
\end{theorem}
\begin{proof}
	Let $m = \set{a_0, …, a_{m - 1}}$.
	Define $k  = \frac{m - 1}{2}$. 
	Observe that the sequences $(a_0, …, a_{k - 2})$ and $(a_{k + 2}, …, a_{m - 1})$ have length $k - 1$ (as $m - 1 - (k + 2) + 1 = 2k - k - 2 + 1$); 
	and the sequences $(a_0, …, a_k)$ and $(a_k, …, a_{m - 1})$ have length $k + 1$. 
	Observe also that $k - 1 ≥ 2$.
	Consider the following profile $P$:
	\begin{equation}
		\begin{array}{*{9}{c}}
			a_0&…&a_{k - 2} & a_{k - 1} & a_k & a_{k + 1} & a_{k + 2} & … & a_{m - 1}\\
			a_{k + 2} & … & a_{m - 1} & a_{k + 1} & a_0 & … & a_{k - 2} & a_{k - 1} & a_k
		\end{array}.
	\end{equation}
	The alternative $a_{k + 1}$ with losses $(k + 1, k - 1)$ is the sole minimizer of dispersion, thus any rule that satisfies $MD$ must include it as a winner, but it is not in the first half.
\end{proof}
\begin{theorem}
	When $m ≥ 10$ is even, there is no $f \in \mathcal{MR}$ that satisfies MD.
\end{theorem}
\begin{proof}
	Let $m = \set{a_0, …, a_{m - 1}}$.
	Define $k  = \khalf$, define $\epsilon = 2k - m + 1 = \khalf - \floor{\frac{m - 1}{2}}$, equivalently, $\epsilon = 1$ iff $m$ is even. Thus $k - \epsilon = \floor{\frac{m - 1}{2}}$.
	Observe that the sequences $(a_0, …, a_{k - 2 - \epsilon})$ and $(a_{k + 2}, …, a_{m - 1})$ have length $k - 1 - \epsilon$ (as $m - 1 - (k + 2) + 1 = 2k - \epsilon - k - 2 + 1$); 
	and the sequences $(a_0, …, a_k)$ and $(a_{k - \epsilon}, …, a_{m - 1})$ have length $k + 1$ (as they are $2 + \epsilon$ longer than the sequences defined just previously).
	Observe also that $k - 1 - \epsilon ≥ 2 + \epsilon$.
	Consider the following profile $P$:
	\begin{equation}
		\begin{array}{*{9}{c}}
			a_0&…&a_{k - 2 - \epsilon} & a_{k - 1 - \epsilon} & a_{k - \epsilon} & [a_{k + 1 - \epsilon}] & a_{k + 1} & … & a_{m - 1}\\
			a_{k + 2} & … & a_{m - 1} & a_{k + 1} & a_0 & [a_1] & a_{1 + \epsilon} & … & a_k
		\end{array}.
	\end{equation}
	The alternative $a_{k + 1}$ with losses $(k + 1, k - 1 - \epsilon)$ and $d(\lprof(a_{k + 1})) = 2 + \epsilon$ is the sole minimizer of dispersion, thus any rule that satisfies $MD$ must include it as a winner, but it is not in the first half.
\end{proof}
\begin{proposition}
	When $m = 8$, the rule $H(\prof) \cap \argmin_{\PE} (d \circ \lprof)$ is well-defined and satisfies MR and MD.
\end{proposition}
\begin{proof}
...
\end{proposition}
\commentRS{Prove the proposition}
...
\commentRS{Define Rawlsian MD as follows: Among the MR alternatives the ones with the minimal dispersion must be among the chosen ones.}

Rawlsian MD implies MR, hence being logically incompatible with MD. 
...
Only x and y are in the first half. Rawlsian minimal dispersion requires y to be among the picked alternatives. 

FB and VR both pick x, thus fail the principle. 

To see that SL fails the principle look at the following profile with 5 alternatives where SL picks a-c while b should be picked.

a b c d e

c b a d e

Using Theorem 2, I think we can show that PV equal and only this satisfies the principle. Note that for m even, the two almost equal PV rules of Theorem will fail the principle. (need to be shown).

However, if we make the principle stronger as "Among the MR alternatives the ones with the minimal dispersion must be the chosen ones" then none of the four rules satisfy it and the principle determines a rule of its own.
