...
\commentOC{I had in mind to obtain the current definition of MR as a formal result rather than as a defintion, something like the following (which unfortunately is too complex to be satisfactory).}
...
\begin{proof}
    ...
    Indeed, we have $a \notin f(\prof)$ iff $v_2 ≥ m - 2$, and $c \notin f(\prof)$ iff $v_1 ≥ m - 2$. Because $v_1 + v_2 ≤ m - 1$ and $m ≥ 4$, we know that $v_1 + v_2 ≤ m - 1 + (m - 4)$. Now if $c \notin f(\prof)$ (equivalently, $- v_1 ≤ -m + 2$), then $v_2 ≤ m - 1 + (m - 4) - m + 2$, then $v_2 ≤ m - 3$, then $a \in f(\prof)$.
\end{proof}
