...
\begin{theorem}
	...
	$\FB$ satisfies PEL, therefore, is PEL-compatible.
	
	Among those, for $m ≥ 3$, only the rules $\PVv[(v_1, v_2)]$ with $\max_{i \in \set{1, 2}} v_i ≤ \frac{m + 1}{3}$ satisfy MD.
\end{theorem}

We end this section by showing that every profile admits some Pareto efficient alternative with a dispersion of at most $\khalf$. Moreover, there is no guarantee of finding an efficient alternative with a dispersion lower than $\khalf$. In other words, $\khalf$ is a general bound on dispersion.

\begin{proposition}
	$[\forall P, \exists x \in \PE \suchthat d(\lprof(x)) ≤ k] ⇔ k ≥ \khalf$.
\end{proposition}
\begin{proof}
    \Cref{th:intersection} establishes, $\forall \prof$, the existence of an alternative $x \in \PE \cap H(\prof)$. Thus $d(\lprof(x)) ≤ \khalf$, proving the "if" part. \commentOC{Do we mean, the "only if" part?} \commentRS{Tried to clarify.}
    To show the "only if" part, consider the profile
	\begin{equation}
		\begin{array}{*8{l}}
...
\begin{table}
	\begin{tabular}{cl*{5}{c}}
		...
		\bottomrule
	\end{tabular}
	\caption{Summary of the results. \commentOC[A tick means that]{Is that an iff?} all members of the corresponding class of rules satisfy the corresponding property (for $m ≥ 4$).
