...
\begin{theorem}
	...
	\commentOC{That’s incorrect, isn’t it? We mean: $[\MRcl \cap \MDcl ≠ \emptyset]$ if and only if $m ≤ 6$ or $m = 8$.}\commentRS{Bravo, you are right.}
\end{theorem}
\begin{proof}
...
\end{definition}
\commentOC{Do we mean $f(\prof) \cap \argmin_{H(\prof) \cap \PE} (d \circ \lprof) ≠ \emptyset$, $\forall \prof \in \allprofs$? Otherwise this seems incompatible with PE.} \commentRS{But don't we exactly say this?}
