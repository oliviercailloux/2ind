...
Let $N = \set{1, 2}$ be the set of individuals and $\allalts$ be the non-empty set of alternatives, with $\card{\allalts} = m$. 
Given $i \in N$, let $\ibar \in N \setminus \set{i}$ denote the other individual.
Let $\powersetz{\allalts}$ denote the non empty subsets of $\allalts$.
Let $\linors$ be the set of linear orders over $\allalts$.
...
An SCR $f$ is anonymous iff $f(\prefi[1], \prefi[2]) = f(\prefi[2], \prefi[1])$.
An SCR $f$ is neutral iff for all permutations $\sigma$ over $\allalts$, $\sigma \circ f(\prof) = f(\sigma \circ \prof)$.

Given $j, l \in \N$, let $\intvl{j, l} = [j, l] \cap \N = \set{k \in \N \suchthat j ≤ k ≤ l}$ denote the interval of integer numbers between $j$ and $l$.
The loss vector of $x$ at $\prof$ is $\lprof(x) \in \intvl{0, m - 1}^N$ that associates to each individual her “loss” resulting of selecting $x$ instead his favorite alternative, that loss being defined as the number of alternatives that the individual prefers to $x$: $\lprof(x)_i = \card{\set{y \in \allalts \suchthat y \prefi x}}$.
...
Let $\PD = \set{x \in \allalts \suchthat \exists y \suchthat \lprof(y) < \lprof(x)}$ be the Pareto-dominated alternatives in $\prof$. Let $\PE = \allalts \setminus \PD$ be the Pareto-efficient ones.
...
The \textbf{Fallback Bargaining} rule picks all alternatives with unanimous support at $\rho$: $\FB(\prof) = U(\prof, \rhoP)$. 

Given a profile $\prof \in \allprofs$, let $H^i(\prof) = \set{x \in \allalts \suchthat \lprof(x)_i ≤ \ceil{\frac{m - 1}{2}}} \subseteq \allalts$ denote the set of alternatives reaching the best half of the ranking of $i$. Let $H(\prof) = H^1(\prof) \cap H^2(\prof) = \set{x \in \allalts \suchthat \lprof(x) ≤ (\ceil{\frac{m - 1}{2}})^N}$ denote the set of alternatives reaching the best half of everybody’s ranking.

The \textbf{Veto-rank} rule $\VR$ is defined only for $m$ odd. Each individual veto her worst $\frac{m - 1}{2}$ alternatives, then the Borda winners among the non vetoed alternatives are picked: $\VR(\prof) = \argmin_{H(\prof)} \sum_N \lprof = \set{x \in H(\prof) \suchthat \forall y \in H(\prof): \sum_N \lprof(x) ≤ \sum_N \lprof(y)}$.

The \textbf{Short listing} rule $\SL$ is defined only for $m$ odd. It picks the best alternative of individual $1$ that is not among the worst $\frac{m - 1}{2}$ alternatives of individual $2$, and the best alternative of $2$ that is not among the worst $\frac{m - 1}{2}$ alternatives of $1$. Formally, $\SL(\prof) = \cup_{i \in N} (\argmin_{x \in H^i(\prof)} \lprof(x)_{\ibar})$.
...
The rule $\PVv = \cap_{i \in N}\set{x \in \allalts \suchthat \lprof(x)_i ≤ a_i} \cap \PE$ picks all alternatives in $\PE$ that nobody vetoed. 
