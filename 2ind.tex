...
\commentOC{I had in mind to obtain the current definition of MR as a formal result rather than as a definition, something like the following (which unfortunately is too complex to be satisfactory).} \commentRS{What you propose is very fine for me. If Matias also agrees, please go ahead. Howeveri the interval from which k comes has an upper bound of k-1, no?}
...
\commentRS{Thanks Matias, I think what you wrote below is fine. I made a few minor changes.}
...
\commentRS{Can we put below a formal proposition claiming that FB, VR, SL and all PV rules except $PV^{=}$ fail RMD. The proof for PV rules is missing but seems to be more or less obvious.}
...
Using Theorem 2, I think we can show that $PV^{=}$ and only this satisfies the principle. Note that for $m$ even, the two almost equal $PV$ rules of Theorem will fail the principle. (need to be shown). \commentMN{I agree}
