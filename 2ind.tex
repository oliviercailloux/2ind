...
\Cref{sec:basic} introduces the basic notions and notation. 
\commentOC{\Cref{sec:lit} defines the rules that we analyze.}
...
Given $p, q \in \R$, let $\intvlz{q} = [0, q] \cap \N $ denote the interval of integers between $p$ and $q$ (throughout the article, $\N$ includes zero). The loss of individual $i$ at $\prof$ for alternative $x$ equals the number of alternatives that $i$ prefers to $x$: $\lprof(x)_i = \card{\set{y \in \allalts \suchthat y \prefi x}}$. The loss vector of $x$ at $\prof$ is denoted $\lprof(x)=(\lprof(x)_1, \lprof(x)_2) \in \lvs$.\commentOC{I am not enthousiastic about putting some of the related  notations in a footnote as done currently. For example, we use the notation $\sum \lprof$ several times in the paper. Why is this one in a footnote and not others? Also, just after the footnote, we use in the main text the notation “$\max \lprof$”, now defined only in a footnote. Finally, one of the reviewers have asked that we clarify some of those notations, which we have now done, so she would probably be unhappy that we now move these clarifications to a footnote. Therefore, I propose to move the footnote back into the main text.}\footnote{Given two loss vectors $l, l' \in \lvs$, $l$ is smaller than $l'$, denoted $l ≤ l'$, iff $l_i ≤ l'_i$ $\forall i$. We also write $l < l'$ when $l$ is strictly smaller than $l'$, meaning, smaller and different.
...
\begin{proposition}
  \label{th:pel}
  For any $m ≥ 3$ \commentOC{Why this restriction? I believe that the statement holds for $m ≥ 2$ and that the proof covers that case.}, a rule $\PVv[(v_1, v_2)] \in \PVcl$ is PEL-compatible iff its veto levels are both at most $\floor{\frac{m}{2}}$, thus, iff $\max_{i \in \set{1, 2}} v_i ≤ \frac{m}{2}$.
\end{proposition}
\begin{proof}
...
\begin{corollary}
  \label{th:pvbpel}
  \commentOC{Same comment: this should be $m ≥ 2$.}
...
\begin{table}
  ...
  \caption{Summary of the results about $\FB$, $\VR$ and $\SL$.
    ...
    \commentOC{I think that it is “for any value of $m ≥ 2$” (or just “for any value of $m$”?).}}
  \label{fig:propsR}
\end{table}
...
\begin{table}
  \begin{tabular}{l*{5}{c}}
    ...
    \bottomrule
  \end{tabular}
  \caption{Summary of the results about the class $\PVcl$ \commentMN{when $m\geq 4$}. Each cell specifies those members that satisfy the corresponding property.}
