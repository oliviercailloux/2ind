...
\commentOC{I find it easier to parse with the logical and (so that the mathematical flow in the brackets is not interrupted), but I won’t insist if you don’t like it.}
...
Given a profile, let us call “RMD alternatives” the alternatives that minimize dispersion among the Paretian alternatives within the first half. As every profile admits RMD alternatives, we can define the Rawlsian minimal dispersion (RMD) principle as the requirement that the social choice always contains an RMD alternative while remaining within the first half.
...
\begin{remark}
\commentMN{	RMD strengthens MR (thus, Strong RMD as well). Also, Strong RMD (thus, RMD) is not stronger than MD as a Paretian minimizer of dispersion might be out of the first half. Consider for example the profile such that}
\commentOC{I do not understand what the wrapping of my previous § within a Matias’ comment means. Do you suggest to remove the sentence? Shouldn’t it be replaced by something, then? Also, I believe that the word “strict” (which was after Paretian) should remain there otherwise the conclusion doesn’t necessarily hold.}
	\begin{equation}
		\begin{array}{llllll}
			a&b&c&d&e&f \\
			d&f&e&a&b&c
		\end{array}.
	\end{equation}
	The alternative $e$ satisfies MD since it minimizes dispersion and is Paretian.
  Yet, a strong RMD rule must pick within the  “first half” alternatives $\set{a, d}$.
	\commentOC{It might be interesting to prove that $\forall m$, Strong RMD is not stronger than MD}.\commentMN{We might want to discuss about the definition of Strong MD (or maybe its name), then. It seems weird to have a stronger notion which turns out not not be stronger..} \commentRS{But I think, strong MR is stronger than MT at every m (except perhaps for small m), no? When m is odd, starting with m at least 7, we can always construct a profile where the MD alternative is not in the first half. A similar statement is valid for m even.}
\commentOC{Sorry, I meant the version that stands now corrected here above (there is no Strong MD).} 
\commentOC{Remzi (just to be sure), with “strong MR” and “stronger than MT”, do you mean respectively “Strong RMD” and “MD”?}
\end{remark}

\begin{theorem}
     $\FB$, $\VR$, $\SL$ and all rules in $\PVcl$ except $\PVe$ fail RMD.
     \commentOC{I find it slightly ambiguous whether the theorem also claims that $\PVe$ satisfies RMD, or only that the rules in PV that are not $\PVe$ do fail it (we want the first interpretation). I suggest the phrasing “$(\set{\FB, \VR, \SL} \cup \PVcl) \cap \RMDcl = \set{\PVe}$”, but I will not complain if you find this not very readable. A perhaps less unreasonably pedantic option is “$\FB$, $\VR$, $\SL$ all fail RMD, and $\PVcl \cap \RMDcl = \set{\PVe}$.”}\commentMN{I agree with the second proposal, maybe removing the word all} \commentRS{I agree with Matias}
\end{theorem}
\begin{proof}
...
\commentRS{The direction below is very promising.}
\commentMN{I agree. Maybe we need a better name than Strong RMD. What about the REF function? REF as Rawlsian (MR), Efficient (Pareto efficient) and Fair ( minimal dispersion)}
\commentOC{We might want to choose the name once we understand it a bit better, perhaps.}
...
\begin{conjecture}
	\commentMN{The Strong RMD rule is not empty and selects at most two alternatives.}
\end{conjecture}
<<<<<<< HEAD
\commentMN{This conjecture seems correct}
\commentOC{Matías or Remzi, what do you mean with “is not empty”?}
\begin{proof}
	Consider any $x \in \argmin_{H(\prof) \cap \PE} (d \circ \lprof)$ and any $i \suchthat \min \lprof(x) = \lprof(x)_i$. Consider any $y \in \argmin_{H(\prof) \cap \PE} (d \circ \lprof)$. We first prove that $\min \lprof(y) = \lprof(y)_i$ iff $y = x$. Indeed, consider any $a \in H(\prof) \suchthat \lprof(a)_i < \lprof(x)_i$, then $\lprof(a)_{\ibar} > \lprof(x)_{\ibar}$ (because $x \in \PE$), so $\lprof(a)_i < \lprof(x)_i ≤ \lprof(x)_{\ibar} < \lprof(a)_{\ibar}$ and $(d \circ \lprof)(a) > (d \circ \lprof)(x)$; and considering now any $a \in H(\prof) \suchthat \lprof(a)_i > \lprof(x)_i$, then $\lprof(a)_{\ibar} < \lprof(x)_{\ibar}$ is required for $a \in \PE$, and if $\min \lprof(a) = \lprof(a)_i$ then $\lprof(x)_i < \lprof(a)_i ≤ \lprof(a)_{\ibar} < \lprof(x)_{\ibar}$ and $(d \circ \lprof)(a) < (d \circ \lprof)(x)$, contradicting $x \in \argmin_{H(\prof) \cap \PE} (d \circ \lprof)$, so that again $\min \lprof(a) ≠ \lprof(a)_i$.
	
	To conclude, observe that if $\argmin_{H(\prof) \cap \PE} (d \circ \lprof)$ contain at least two elements (say, $x$ and $y$), one of them, say, $x$, has $\min \lprof(x) = \lprof(x)_i$, the other one has $\min \lprof(y) = \lprof(y)_{\ibar}$, and thus there can’t be a third element $z$ in $\argmin_{H(\prof) \cap \PE} (d \circ \lprof)$ as neither $\min \lprof(z) = \lprof(z)_i$ nor $\min \lprof(z) = \lprof(z)_{\ibar}$ is possible, by the above result.
\end{proof}
\commentOC{This proof seems perhaps too complex compared to the relative intuitiveness of this result but I can’t see a simpler one for now. Any idea?}
\commentMN{This conjecture seems correct. On the following extension, why do we want to refine it (i.e. make it more resolute)? Maybe anoninimty is a nice property but the strong RMD rule is anonymous, right?}
...
\begin{conjecture}
	That refined Strong RMD rule is a most resolute rule that is symmetric (anonymous and neutral) and Strong RMD.
\end{conjecture}
\begin{conjecture}
	That refined Strong RMD rule is the sole rule that is anonymous and Strong RMD and responsive to losses domination.
\end{conjecture}
