...
\begin{abstract}
    We consider two-person ordinal collective choice from an axiomatic perspective. We explore two principles: minimal Rawlsianism (the alternative chosen should be in the upper-half of both individuals) and the equal loss principle (both individuals should concede as equally as possible from their highest ranked alternative). We consider most of the known rules in the literature and identify which of these two principles they satisfy. Since these principles are roughly speaking incompatible, we introduce the
Rawlsian minimal dispersion principle that requires the outcome to contain
the Rawlsian alternatives whose difference between the ranks is minimized which is satisfied by a unique rule in our clas.
\end{abstract}
...
To see why the claim holds, consider the next profile:
	\begin{equation}
		...
		a_2& a_1 &a_3&a_4 &\ldots&a_m \\
		\end{array}.
	\end{equation}
  Observe that alternatives $a_1$ and $a_2$ Pareto dominate any other alternative in the profile so that $a_1,a_2$ are the only Pareto efficient alternatives. If $m=3$, $a_3$ is the only alternative that satisfies equal loss; it is also the only Pareto dominated alternative. When $m>3$, a similar idea applies: each alternative in  $\{a_3,\ldots,a_m\}$ satisfies equal loss and is Pareto dominated while both $a_1,a_2$ are Pareto efficient while failing equal loss since they are ranked at the same level by both individuals. Both observations, when $m=3$ and $m>3$, imply the incompatibility of both principles.
\end{proof}
\commentRS{Thanks for the proof Matias. The way it is presented requires at least four alternatives while the theorem also covers the case of three alternatives. The profile should be changes so as to cover this as well.}\commentMN{Thanks Remzi, I added a modification of the proof to take into account your comment, it should be ok now.}
...
\end{proof}
\commentRS{Should we add a corollary about binding PV rules that satisfy PEL?}\commentMN{Not so sure}
\commentOC{I add the corollary here below just to have an idea of what it looks like, then we can decide.}\commentMN{Thanks Olivier, we can add the corollary, I let both of you decide.}
...
The following remark will be useful to explore the precise relationship between MD and MR.
\begin{remark}
	\label{remark1} 
...
$\lprof(x)_1+\lprof(x)_2\geq m$. Thus, $\#\{y\in \mathcal{A}\mid y \succ_1 x\}+\#\{y\in \mathcal{A}\mid y \succ_2 x\}\geq m$, which implies $\{y\in \mathcal{A}\mid y \succ_1 x\}\cap \{y\in \mathcal{A}\mid y \succ_2 x\}\neq \emptyset$, contradicting $x \in \PE$.
	...
	\commentRS{You are right, if we discard the result with stronger MD.}
	\commentOC{Thinking again, maybe this result deserves its own headline. It is simple but quite useful for the reader to observe, even though we happen to use it only once. But I have no strong opinion about it.},
	\commentRS{I am happy to keep it.}\commentMN{Me too}
	\commentOC{I suggest to make it a theorem. If you agree, I’ll change it accordingly.}
\end{remark}
...
\commentOC{Consider externalizing the implication: $m ≥ 3$ and $t ≥ \frac{2m - 4}{3}$ ⇒ $t ≥ \khalf$.}\commentMN{See my next comment}
\begin{theorem}
...
%	By construction thus, $\forall a ≠ y \in A: \lprof(y)_{\ibar} < \lprof(a)_{\ibar}$, hence, $\lprof(y)_{\ibar} ≤ m - 1 - \card{(A \setminus \set{y})} = m - 1 - t$.
	...
	It follows that $\lprof(x)_{\ibar} < \lprof(y)_{\ibar}$, otherwise $y$ Pareto-dominates $x$; whence $\lprof(y)_i < \lprof(y)_{\ibar}$ otherwise $\lprof(x)_{\ibar} < \lprof(y)_{\ibar} ≤ \lprof(y)_i < \lprof(x)_i$ and $d(\lprof(y)) < d(\lprof(x))$, contradicting $x \in \argmin_{\PE} (d \circ \lprof)$.
	 ...
	 Observe that as $t \in \N$, $t ≥ \frac{2m - 4}{3} ⇒ t ≥ \ceil{\frac{2m - 4}{3}}$, and when $m ≥ 3$, $\ceil{\frac{2m - 4}{3}} ≥ \khalf$; whence $t ≥ \khalf$\footnote{\commentMN{I suggest simply putting it as a footnote:}When $m$ odd, $m ≥ 5 ⇒ \frac{2m - 4}{3} ≥ \frac{m - 1}{2} = \khalf$ and when $m$ is even, $m ≥ 8 ⇒ \frac{2m - 4}{3} ≥ \frac{m}{2} = \khalf$; when $m = 3$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{2}{3}} = 1 = \khalf$; when $m = 4$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{4}{3}} = 2 = \khalf$; when $m = 6$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{8}{3}} = 3 = \khalf$.} \commentOC{I ignore how to prove that shortly, here are the gory details. When $m$ odd, $m ≥ 5 ⇒ \frac{2m - 4}{3} ≥ \frac{m - 1}{2} = \khalf$ and when $m$ is even, $m ≥ 8 ⇒ \frac{2m - 4}{3} ≥ \frac{m}{2} = \khalf$; when $m = 3$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{2}{3}} = 1 = \khalf$; when $m = 4$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{4}{3}} = 2 = \khalf$; when $m = 6$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{8}{3}} = 3 = \khalf$}
...
\commentOC{We decided to leave the theorem above as-is and discuss this theorem verbally, and point out that 2 equiv 3 can also be seen more directly, just by anonymity. I tried a first draft in the following remark, which has to be better phrased.}
\commentMN{I tried for the 1st two paragraphs. I think that the third one is really hard, we can either simply drop it or simplify it.}
\begin{remark}
	The equivalence $1 ⇔ 2$ says that in order ensure that there is an alternative within the first $t$ alternatives for both players that minimizes dispersion among the Pareto ones, it is necessary and sufficient to set $t$ at least $\frac{2m - 4}{3}$. As a matter of fact, by the anonymity of $\argmin_{\PE} (d \circ \lprof)$, ensuring this existence for the first $t$ alternatives for both players is equivalent to ensure it for the first $t$ alternatives for one of the players, as expressed by the equivalences $2 ⇔ 3$ and $2 ⇔ 4$. Thus, considering $1 ⇔ 3$ or $1 ⇔ 4$,  in order to ensure  that an alternative that minimizes dispersion among the Pareto ones exists within the first $t$ alternatives for one of the players, it is necessary and sufficient to set $t$ at least $\frac{2m - 4}{3}$.
