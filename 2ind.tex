...
\commentRS{I made the changes proposed by Olivier and agreed by Matias. A question: why we say k-bounded? What kind of a bound we mean. Another issue: For proposition 1, we must cite the related result in Brams and Kilgour.}

\begin{definition}[$k$-bounded Rawlsianism] Given $k \in \intvl{0, m - 1}$, an SCR is $k$-bounded Rawlsian iff it selects its winners among those alternatives whose losses are within $\intvl{0, k}$ for both voters. Formally:
...
\begin{proposition}
	Given $k  < \khalf$, no SCR is $k$-bounded Rawlsian.
\end{proposition}
...
\end{theorem}
 \begin{proof}
	For $\VR$ and $\SL$, the proof follows from the definition of the SCRs since each of them selects only alternatives among the top-half alternatives of both individuals. For $\FB$, Theorem 1 of \cite{BramsKilgour2001} shows that $\forall \prof \in \allprofs$, $\rhoP ≤ \ceil{\frac{m - 1}{2}}$.
...
\commentRS{We had issues about the proofs of Propositions 7 and 8 which seem to be fixed.}
...
Regarding $PV$ rules, we first show that for $m$ even, $\PVv[(\frac{m}{2}, \frac{m}{2} - 1)]$ and $\PVv[(\frac{m}{2} - 1, \frac{m}{2})]$ both fail RMD. To see this, consider the profile \mathbf{P}
