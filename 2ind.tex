...
\begin{proof}
We use a proof strategy that establishes the equivalence if four statements, namely the two stated by the theorem and also the following two statements which we respectively denote by 3 and 4: $\forall \prof: \lprofinv(\intvl{0, t} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset$
and \label{it:Ptbig1} $\forall \prof: \lprofinv(\intvl{0, m - 1} × \intvl{0, t}) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset$. The proof establishes the equivalence of 1, 2, 3 and 4.


	For the claim $\ref{it:tbound} ⇒ 2$,  consider any $t ≥ \frac{2m - 4}{3}$ .  We now show that the set $\lprofinv(\intvl{0, t} × \intvl{0, t}) \cap \argmin_{\PE} (d \circ \lprof)$ is non-empty for any profile $\prof$. Consider any $x \in \argmin_{\PE} (d \circ \lprof)$, and let $i$ denote any individual such that $\max \lprof(x) = \lprof(x)_i$. 
 ...
 and hence $d(\lprof(x)) ≥ m - t - 1 ≥ d(\lprof(y))$, which implies that $y \in \argmin_{\PE} (d \circ \lprof)$.
	...
	The claims $\ref{it:Pt} ⇒ 3$ and $\ref{it:Pt} ⇒ 4$ and hold thanks to set inclusion.
	
	Turning now to the claim $3 ⇒ \ref{it:tbound}$ (a similar claim shows that $3\Longrightarrow 4$ by exchanging the role of individuals), consider some $t < \frac{2m - 4}{3}$ so that 1. fails. Our aim is to prove that there is some profile $\prof$ such that $\lprofinv(\intvl{0, t} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$, violating 3. To this end, we define $t' = \max \set{\khalf, t}$.
 
	Observe that $m = t' + 1 + (m - t' - 2) + 1$.  We label the alternatives as $\set{a_1, …, a_{t' + 1}, c_1, …, c_{m - t' - 2}, x}$ and consider the sequences $A = (a_1, …, a_{t' + 1})$ and $C = (c_1, …, c_{m - t - 2})$. Assume that the preferences of individual $1$ (resp. individual 2) are $(A, x, C)$ (resp.  $(C, x, A)$).
 ...
 Thus, we have proved that there is some profile $\prof$ such that $\lprofinv(\intvl{0, t'} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$. Since $t'\geq t$ by construction, it follows that 
there is some profile $\prof$ such that $\lprofinv(\intvl{0, t} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$, proving that 3. fails to hold, as desired.
 
\end{proof}

\begin{remark}
	By the anonymity of $\argmin_{\PE} (d \circ \lprof)$, ensuring the existence of an alternative within the first $t$ alternatives for both players is equivalent to ensure it for the first $t$ alternatives for one of the players, as expressed by the equivalences $2 ⇔ 3$ and $2 ⇔ 4$. Thus, considering $1 ⇔ 3$ or $1 ⇔ 4$,  in order to ensure  that an alternative that minimizes dispersion among the Pareto ones exists within the first $t$ alternatives for one of the players, it is necessary and sufficient to set $t$ at least $\frac{2m - 4}{3}$.
\end{remark}
