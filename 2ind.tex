...
The minimal Rawlsian and minimal dispersion principles being logically incompatible, the SCRs that satisfy MR (namely $\FB$, $\VR$, $\SL$ and the rules in $\PVcl$ identified by \cref{th:pvmr}) all fail MD. Since \cref{th:pvmr} shows that most rules in $\PVcl$ fail MR, the following result determines the subclass of $\PVcl$ satisfying MD. 
...
Define the rule $MD(\prof) = \argmin_{\PE}(d \circ \lprof)$ which selects every Pareto efficient alternative that minimizes the dispersion. This rule is the finite version of \possessivecite{thomson1994cooperative} equal area rule in our framework with no disagreement outcome. 

\begin{remark}
The rule $MD(\prof)$ is not in $\PVcl$, as the following example shows.
