...
\begin{itemize}
	...
	\item The \textit{veto-rank mechanism} where, given an odd number $m$ of alternatives, each individual vetoes $(m - 1) / 2$ alternatives and ranks the remaining $(m+1) / 2$. The outcome is the alternative with the minimal sum of ranks among those that have not been vetoed. \commentOC{the set of alternatives… ?}
	\item The \textit{shortlisting procedure} where one individual selects $(m+1) / 2$ alternatives and the other individual decides on the outcome out of that shortlist.
	\item The class of \textit{Pareto-and-veto rules} where each individual $i$ vetoes a fixed number $v_i$ of alternatives with $v_1 + v_2$ being lower than the total number of alternatives $m$. The outcome is the set of Pareto optimal alternatives that are not vetoed. 
\end{itemize}
...
\begin{table}
	\begin{tabular}{l*{6}{c}}
		...
		$\PVe$ & ✓ & ✓ & & & & ✓
	\end{tabular}
	\caption{Properties satisfied (for $m ≥ 4$)}
	\label{fig:props}
\end{table}
...
Given two loss vectors $l, l' \in \lvs$, we say that $l$ is weakly smaller than $l'$, $l ≤ l'$, iff $\forall i: l_i ≤ l'_i$. We also write $l < l'$ to denote that $l$ is strictly smaller than $l'$, meaning, weakly smaller and different. Let $\min \lprof(x) = \min_{i \in N} \lprof(x)_i \in \N$ 
...
\commentOC{Perhaps it’s overly prudent, but because, sorry, since the notion of the class of rules is crucial in this article, I suggest to add a sentence to make the property stand out and specify better what we mean with class of rules satisfying a property. We say that $f$ satisfies the Pareto property iff it picks only Pareto efficient alternatives.}
...
In concordance with the ceiling established by Theorem 1 of \citet{BramsKilgour2001}, we use the term “half” to mean the smallest integer $k$ that exceeds $m-k$ \commentOC{Isn’t it more correct to write something like: to designate a portion of the profile such that…? I am not sure.} and for every profile $\prof \in \allprofs$, we let $H(\prof) = \set{x \in \allalts \suchthat \lprof(x) ≤ (\ceil{\frac{m - 1}{2}}, \ceil{\frac{m - 1}{2}})}$ denote the set of alternatives reaching the best half of every individual’s preference. Given $\prof$ and a loss level $k \in \losses$, define $U(\prof, k) = \set{x \in \allalts \suchthat \lprof(x) ≤ (k, k)}$ as the set of alternatives imposing losses not higher than $k$ for all individuals. 
...
\commentOC{I suggest inlining the formal definition or just writing “thus”.}
\commentOC{$H^i$ is not defined, and perhaps the following alternative would be no more obscure: $x \suchthat \lprof(x)_i ≤ \floor{\frac{m - 1}{2}}$ instead of $x \in H^i(\prof)$ (with the slight drawback that it does not highlight the use of “half”)? Alternatively, define $H(P_i)$ as the upper half of someone’s preference (but it’s cumbersome).}
...
Given $v_i \in \intvl{0, m - 1}$, define $a_i = m - v_i - 1 \in \intvl{0, m - 1}$ as the highest acceptable loss level for individual $i$. For $v=(v_1,v_2)$, the rule $\PVv = \cap_{i \in N}\set{x \in \allalts \suchthat \lprof(x)_i ≤ a_i} \cap \PE$ picks all Pareto-efficient alternatives that no individual vetoes. 
...
\begin{definition}[$k$-Rawlsianism] 
	\label{def:kr}
	Given $k \in \intvl{0, m - 1}$, an SCR is $k$-Rawlsian iff it selects its winners among those alternatives whose losses are within $\intvl{0, k}$ for both individuals. Formally:
...
\end{theorem}
 \begin{proof}
	We start by claiming that $\PVcl ∩ \MRcl = \set{\PVv \in \PVcl \suchthat \forall i: v_i ≥ \floor{\frac{m - 1}{2}}}$. 
	\commentOC{To ease the comparison with \cref{th:pvbpel}, I’d write this equivalent claim in the theorem part, in supplement to the claim as it is now.}
	To show the “if” part of the claim, note that given any profile, the condition $\forall i: v_i ≥ \floor{\frac{m - 1}{2}}$ suffices to guarantee that $\PVv(\prof) \subseteq H(P)$. To see the “only if” part, consider an arbitrary ordering $\prefi$ over $\allalts$, let $\prefiinv$ denote its inverse, and consider the profile $\prof = (\prefi, \prefiinv)$.
	Observe that $\PVv(\prof)$ will exclusively pick winners in the first half of individual $i$ only if $v_i ≥ \floor{\frac{m - 1}{2}}$.
	\commentOC{Are we leaving too much work to the reader here?}
...
\end{theorem}
\begin{proof}
	Consider the following profile $\prof$, reused from \eqref{eq:distinct}, where $\VR(\prof) = \set{b}$ and $\SL(\prof) = \set{a, g}$:
	\begin{equation}
		\begin{array}{llll lll | llll ll}
			a&b&c&d&e&f&g&h&i&j&k&l&m\\
			g&h&i&d&b&j&a&c&e&f&k&l&m\\
...
\begin{proof}
	...
	\commentOC{Mmmh this is our result about Pareto again, right? Should we move it?}
...
\commentOC{This has to be (re)moved, as we use it as soon as in \cref{def:kr}.}
...
%	  \footnote{When $m$ odd, $m ≥ 5 ⇒ \frac{2m - 4}{3} ≥ \frac{m - 1}{2} = \khalf$ and when $m$ is even, $m ≥ 8 ⇒ \frac{2m - 4}{3} ≥ \frac{m}{2} = \khalf$; when $m = 3$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{2}{3}} = 1 = \khalf$; when $m = 4$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{4}{3}} = 2 = \khalf$; when $m = 6$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{8}{3}} = 3 = \khalf$.} 
	 Observe that as $t \in \N$, $t ≥ \frac{2m - 4}{3} ⇒ t ≥ \ceil{\frac{2m - 4}{3}}$.
	 Furthermore,
	$\ceil{\frac{2m - 4}{3}} = \frac{2m - 4 + \epsilon}{3}$ with $\epsilon = 0$ iff $\frac{2m - 4}{3} = 0$ iff $\frac{2m - 1}{3} = 0$; $\epsilon = 1$ iff $\frac{2m - 6}{3} = 0$ iff $\frac{m}{3} = 0$; and $\epsilon = 2$ iff $\frac{2m - 5}{3} = 0$ iff $\frac{m - 1}{3} = 0$.
	This permits to see that $\ceil{\frac{2m - 4}{3}} ≥ \frac{m - 1}{2}$.
