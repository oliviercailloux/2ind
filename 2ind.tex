...
\affil[**]{CREST, CNRS, \'Ecole Polytechnique, GENES, ENSAE Paris, Institut Polytechnique de Paris, 91120 Palaiseau, France.}
\hypersetup{
	pdfsubject={},
	pdfkeywords={},
}
...
In concordance with the ceiling established by Theorem 1 of \citet{BramsKilgour2001}, we use the term “half” to mean the smallest integer $k$ that exceeds $m-k$ \commentOC{Isn’t it more correct to write something like: to designate a portion of the profile such that…? I am not sure.} and for every profile $\prof \in \allprofs$, we let $H(\prof) = \set{x \in \allalts \suchthat \lprof(x) ≤ (\ceil{\frac{m - 1}{2}}, \ceil{\frac{m - 1}{2}})}$ denote the set of alternatives reaching the best half of every individual’s preference. Given $\prof$ and a loss level $k \in \losses$, define $U(\prof, k) = \set{x \in \allalts \suchthat \lprof(x) ≤ (k, k)}$ as the alternatives imposing losses not higher than $k$ for all individuals. 
...
\end{remark}
\begin{remark}
    $\FB$, $\VR$ and $\SL$ are anonymous, while a $\PVv \in \PVcl$ is anonymous iff $v_1 = v_2$.
\end{remark}
...
\begin{proof}
	...
	\begin{equation}
		...
		\end{array}.
	\end{equation}
    SPEL requires to pick solely $b$. Any rule $f \in \PVcl$ will pick either $a$ or $c$ in addition to $b$. 
    Indeed, we have $a \notin f(\prof)$ iff $v_2 ≥ m - 2$, and $c \notin f(\prof)$ iff $v_1 ≥ m - 2$. Because $v_1 + v_2 ≤ m - 1$ and $m ≥ 4$, we know that $v_1 + v_2 ≤ m - 1 + (m - 4) = m - 2 + m - 3$. Now if $c \notin f(\prof)$ $\Longleftrightarrow$ $m - 2 ≤ v_1$, then $v_1 + v_2≤ v_1 + m - 3$, then $a \in f(\prof)$.
\end{proof}
