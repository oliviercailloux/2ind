...
\label{sec:minprinc}
This section focuses on the notion of Rawlsianism, which aims at \commentOC[maximizing the welfare of the worst-off individual]{This wording suggests the rule that, given a profile, minimizes the worst loss, thus, $\FB$; whereas we mean that this principle gives an a priori bound on losses that, given a profile, will be satisfied if possible. In hindsight, I do not think that the name “Rawlsian” for this is deserved, as Rawlsianism to me invokes an idea of “worst off is as good as possible” (that is, $\FB$), rather than “a priori bounded loss” (though I agree with the term “minimal Rawlsianism” as it is weaker than a truly Rawlsian principle). But if you think that it is now too late to change the name (which admittedly would be boring now that we are about to submit), I will not object. Anyway, we can at least slightly change the verbal description of it, I suppose}. \commentRS{Isn't it unfair to qualify the bound imposed by the MR principle as "a priori"? After all, this bound of "half" is a consequence of Rawlsianisn and UC, which makes me feel comfortable with calling the concept "MR".}
\commentOC{Maybe the word “a priori” is not most appropriate, I meant that the bound is given so as to work for all profiles, whereas one could also logically desire that, given the profile, we pick the alternative that minimizes the worst loss. What do you mean with UC as a principle, in “this bound of "half" is a consequence of Rawlsianisn and UC”? (In my previous comment, I was referring to “Unanimity Compromise”, that is, FB.)}\commentMN{I really like MR for Rawlsianism, I think that many people like to think of this in this manner, I would leave it as it is.} \commentRS{So MR wins with a majority!}
\begin{definition}[$k$-Rawlsianism] 
	\label{def:kr}
	Given $k \in \intvl{0, m - 1}$, a SCR is $k$-Rawlsian iff it selects among alternatives whose losses are within $\intvl{0, k}$ for both individuals, whenever such alternatives exist. 
...
%	Thus, suffices to show that for some $t' ≥ \ceil{\frac{2m - 7}{3}}, \exists \prof \suchthat \lprofinv(\intvl{0, t'} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$. 
...
\end{proposition}
\begin{proof}
    \Cref{th:intersection} establishes, $\forall \prof$, the existence of an alternative $x \in \PE \cap H(\prof)$. Thus $d(\lprof(x)) ≤ \khalf$, proving the “if” part. To show the "only if" part, consider the profile
	\begin{equation}
		\begin{array}{*8{l}}
...
\Cref{fig:propsR,fig:propsC} summarize our findings.\footnote{It should be noted that the axioms listed on \Cref{fig:props} do not characterize the SCRs that we discuss. This is somehow expected, as these axioms impose conditions at a given profile without alluding to the consequences when the given profile is modified, i.e., they are “punctual” in the sense of \citet{thomson2012axiomatics}. We thank Sylvain Béal for this comment.}
\commentOC{What do you think about replacing \cref{fig:propsC} by \cref{fig:propsCexplicit}?} \commentRS{I would go for a mixture! Table 2 for MR and RMD; Table 3 for PEL-compatible and MD.} \commentMN{But a mixture would imply three tables in the same conclusion which seems a lot, right? I am not sure of the difference between Table 2 and 3, with Table 3 being slightly better. } \commentRS{I didn't mean an additional table. I was thinking of one table that combines some features of Table 2 and 3. Anyway, I don't have any strong feeling about this. Whatever you prefer is fine for me.}
