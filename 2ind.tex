...
\noindent \textbf{Remark 1:} For any $\prof$, if $x\in PE(\prof)$ then
...
\begin{theorem}
	Given $t \in \intvl{0, m - 1}$, $m$ is “small enough” implies $[\forall \prof: \lprofinv(\intvl{0, t} × \intvl{0, t}) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset]$.
\end{theorem}
\begin{proof}
	Consider any $x \in \argmin_{\PE} (d \circ \lprof)$, and let $k = \max \lprof(x)$ denote its worst loss and $i$ any voter such that $k = \lprof(x)_i$. The alternative $x$ thus has $k$ alternatives better than it for the voter $i$, and therefore must have at least those $k$ alternatives worst than it for the other voter (otherwise it is Pareto-dominated by at least one of those $k$ alternatives), say, $k + r$. Thus $\lprof(x)_{\ibar} = m - 1 - k - r$. In other words, $\lprof(x)(1, 2) = \set{k, m - 1 - k - r}$, with $k ≥ m - 1 - k - r$. 
	It follows that $d(\lprof(x)) = 2k + 1 + r - m$.
	
	Let $A = \set{a \suchthat \lprof(a)_i ≤ t}$ designate those first ones for $i$. 
	If $k ≤ t$, the proof is done. Otherwise, if $m$ is small enough, some alternative $y \in A$ has a not greater dispersion than $x$ (hence non empty set, containing $y$).
	Write $\epsilon = k - t$ the surplus alternatives. Write $y = \argmin_A \lprof(.)_{\ibar}$ the best alternative for voter $\ibar$ among $A$. The worst case in terms of dispersion (biggest dispersion of the alternatives within $A$) is when $\lprof(y)_1 = 0$, in which case $d(y) = \lprof(y)_2$, which is biggest at $1 + (m - 1 - k) = m - k$, as it may be preceded by $x$ and the alternatives not being $x$ nor $k$ (and not $\epsilon$ if $t < m - k$, because any of these there would have dispersion strictly lower than $x$, having interval of losses within the interval of losses of $x$, $\lprof(x)_{\ibar} < \lprof(e)_{\ibar} ≤ \lprof(e)_i < \lprof(x)_i$, whereas if $t ≥ m - k$ then $\lprof(e)_{ibar} ≥ \lprof(e_i)$ and this reasoning does not hold, to be checked). 
	In other words, $y$ is only followed by $k - 1$ alternatives thus its loss is $(m - 1) - (k - 1)$. Note that the other alternatives in $A$ have non lower dispersion than $y$. When is $d(y) > d(x)$ (equiv, when is the set empty)? When $m - k > 2k + 1 + r - m$. Choosing $r = 0$ and $k = t + 1$, this is possible iff $2m > 3t + 4$, equivalently, $m ≥ \frac{3t + 5}{2}$. For example, when $t = \khalf$, $m$ odd, we obtain $m ≥ \frac{3 (m - 1) + 10}{4}$ thus $m ≥ 7$. 
...
%	In other words, $y$ is only followed by $t - 1$ alternatives thus its loss is $(m - 1) - (t - 1)$. Note that the other alternatives in $A$ have non lower dispersion than $y$. When is $d(y) > d(x)$ (equiv, when is the set empty)? When $m - t > 2k + 1 + r - m$. Choosing $r = 0$ and $k = t + 1$, this is possible iff $2m > 3t + 3$, equivalently, $m ≥ \frac{3}{2}t + 2$.
...
\commentRS{I restated Proposition 11 according to the stronger definition of MD. I wrote the proof of the necessity. Can you please check and also correct latex errors. Once you agree, I'll send the proof of the sufficiency.}
...
For $r=2$, consider the profile $\prof''$
...
where $PE(\prof'' )=\{a_{1}, a_{2k+1}, x\}$ while
 $d(\lambda_{\prof''}(a_{1})) =k+1$, $d(\lambda_{\prof''}(a_{2k+1})) = 2k+2$ and $d(\lambda_{\prof''}(x)) = k+1$. Thus, any SCR that satisfies
MD must pick $x$, which is, however, vetoed by individual 1. 
...
\commentRS{As a general concern about the presentation of our results, we must decide what is a proposition what is a theorem.}
