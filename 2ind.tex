...
\commentRS{I mentioned "majoritarian approval" and "majoritarian optimality" in the above paragraph but didn't apologize for not using these terms instead of "minimal rawlsianism" as the two concepts are related but different.}
...
\commentRS{In a footnote at the end of the paragraph above, we can say the following: In a two-person collective choice framework with an interpretation that is more specified than ours, \cite{Clippel} call this condition the \textit{minimal satisfaction test}.}
...
\commentRS{For proposition 1, we must cite the related result in Brams and Kilgour.}
...
\end{definition}
As noted in the introduction, $k$-Rawlsianism can be satisfied only when $k$ is at least “half”.
\begin{proposition}
	Given $k  < \khalf$, no SCR is $k$-Rawlsian.
\end{proposition}
...
\commentOC{Can somebody try to phrase this better?}
\commentRS{I made an attempt.}
\begin{definition}[Minimal Rawlsianism (MR)] An SCR $f$ satisfies MR if 
	$\forall \prof \in \allprofs,  f(\prof) \subseteq H(\prof)$.
\end{definition}

\commentOC{I took care of consistency for typography of classes of rules, properties, rules (I may have missed some). Not sure how to deal with the property VS class typography, for example we often write “fail SPEL”. Also, we use $\PE$ for the rule and it also suggests the property, is this all right?}
Let $\MRcl$ denote the class of minimally Rawlsian (MR) SCRs.
Similarily, we systematically use caligraphic letters to denote the class of rules satisfying a given property.
...
\commentOC{Below is an attempt of phrasing a general proposition from which \cref{th:noMRMD} and \cref{th:MRMD} follow. But I can’t come up with a united proof. As a possible consolation, I added a sentence in the proof of \cref{th:noMRMD} (as first sentence) which helps seeing how \cref{th:noMRMD} and \cref{th:MRMD} relate.}
\begin{proposition}
	The property $\forall \prof: H(\prof) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset$ is satisfied exactly when $m = 8$ or $m ≤ 6$.
\end{proposition}
...
\end{theorem}
\begin{proof}
	We will define a profile $P$ such that $H(\prof) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$.
	
	Let $m = \set{a_0, …, a_{m - 1}}$.
	Define the sequences $S_1 = (a_0, …, a_{\khalf})$ and $S_2 = (a_{\khalf + 2}, …, a_{m - 1})$; define $k = \khalf + 1$.
	Observe that $S_1$ has length $\khalf + 1$ and $S_2$ has length $(m - 1) - (\khalf + 2) + 1 = (m - 1 - \khalf) - 1 = \floor{\frac{m - 1}{2}} - 1$.
...
%	and the sequences $S_1 = (a_0, …, a_{\khalf})$ and $(a_{\floor{\frac{m - 1}{2}}}, …, a_{m - 1})$ have length $\khalf + 1$.
	Define the profile $P$ as containing for the first voter the preference ordering $(a_0, …, a_{m - 1})$ and for the second voter the ordering defined as $S_2$ followed by $a_k$ followed by $S_1$.
	...
	The alternative $a_k$ is preceded, in the preference of $v_2$, by the alternatives in the sequence $S_2$ of length $\floor{\frac{m - 1}{2}} - 1$, thus, $\lprof(a_k)_2 = \floor{\frac{m - 1}{2}} - 1$.

	It follows that the alternative $a_k$, with losses $(\khalf + 1, \floor{\frac{m - 1}{2}} - 1)$ and $d(\lprof(a_{\khalf + 1})) = 2 + \khalf - \floor{\frac{m - 1}{2}}$ is the sole minimizer of dispersion. Indeed, the alternatives $a_j \in S_1$ have losses $(j, j + \floor{\frac{m - 1}{2}})$, thus with a strictly greater dispersion $\floor{\frac{m - 1}{2}}$ (as $\floor{\frac{m - 1}{2}} ≥ 3 + \khalf - \floor{\frac{m - 1}{2}}$ thanks to the bounds in our hypothesis); and the alternatives $a_{\khalf + 2 + j} \in S_2$ have losses $(\khalf + 2 + j, j)$, thus with a strictly greater dispersion $\khalf + 2$.
...
\end{proof}
\commentRS{It will be much easier to follow the proof if we present it for m being even and odd separately. What do you think?}
\commentOC{Each proof would be easier to read, but readers would have two proofs to check, which perhaps compensates for the additional simplicity. Also, the current presentation explains the (a priori strange) behavior around $m = 8$, as $m$ has to satisfy $\floor{\frac{m - 1}{2}} ≥ 3 + \khalf - \floor{\frac{m - 1}{2}}$ (failing for $m ≤ 6$, satisfied for $m = 7$, failing for $m = 8$ and satisfied again for $m ≥ 9$). (BTW perhaps this deserves an explicit comment.) And this general presentation may permit further general (I mean, not case-separated according to parity) results, whereas if we split the proof, a reader will have to come up herself with a general one if she wants to go back to a non-case-separated approach, which is (much) more difficult to do than the reverse process. However I agree that it would be nice to ease reading. I attempted to define the profiles using sequences only as I think displaying it might be more confusing than helpful (perhaps numbering from $0$ instead of $1$ would help). I can restore the previous version if this is not a good idea. Alternatively, perhaps we could somehow specify (in a footnote?) what the profile $P$ looks like in each case (odd and even)?}

The following proposition will be useful to prove \cref{th:MRMD}.
\begin{proposition}
	\label{th:maxDisp}
	Given any profile $\prof$, $\exists x \in H(\prof) \suchthat d(\lprof(x)) ≤ \khalf$.
	That bound is strict, in other words, $\forall m, \exists \prof \suchthat \forall x \in H(\prof): d(\lprof(x)) ≥ \khalf$.
\end{proposition}
\begin{proof}
	About the first claim, consider the $\khalf + 1$ alternatives having losses for the first voter $\lprof(x)_1 ≤ \khalf$.
	At least one of those alternatives will have a loss for the second voter within $\intvl{0, \khalf}$ (as the complementary interval of possible losses is $\intvl{\khalf + 1, m - 1}$ which has cardinality $\floor{\frac{m - 1}{2}} < \khalf + 1$). Such alternatives have dispersion $d(\lprof(x)) ≤ \khalf$.

	About the second claim, define $\allalts = \set{a_1, …, a_m}$, define $S_1$ as the sequence $(a_1, …, a_{\floor{\frac{m + 1}{2}}})$ and $S_2$ as the sequence $(a_{\floor{\frac{m + 1}{2}} + 1}, …, a_m)$, and define $\prof$ as containing the preference ordering $a_1, …, a_m$ for voter $1$ and $S_2$ followed by $S_1$ for voter $2$. Alternatives in $S_2$ have dispersion $\floor{\frac{m + 1}{2}}$ (the length of $S_1$) and alternatives in $S_1$ have dispersion $m - (\floor{\frac{m + 1}{2}} + 1) + 1 = (m + 1) - \floor{\frac{m + 1}{2}} - 1 = \ceil{\frac{m + 1}{2}} - 1 = \ceil{\frac{m - 1}{2}}$ (the length of $S_2$).
\end{proof}
...
\Cref{th:maxDisp} shows that for $m ≤ 6$, at least one alternative in the first half has a dispersion that is not greater than the one of $x$. Such alternative thus belongs to $H(\prof) \cap \argmin_{\PE} (d \circ \lprof)$. This argument also applies to $m = 8$ if $d(\lprof(x)) = 4$. Remains to check the case $m = 8$ and $d(\lprof(x)) = 3$.
\commentOC{Not sure how to determine conclusively and clearly that $H(\prof) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset$ in that case.}
\end{proof}
	
\commentRS{We agreed to start with a result on the emptiness (or not) of $H(\prof) \cap \argmin_{\PE} (d \circ \lprof)$ depending on the value of $m$; \cref{th:noMRMD} and \cref{th:MRMD} will follow as corollaries. Olivier will take care of this.}
...
\end{proposition}
\commentRS{The necessity part of the proof is below. Matias, thanks for typing it but what you wrote is different than what I sent. Can you check? Also, can you give a hand to improve the style and make certain expressions more formal. I'll send a sketch of the sufficiency part.}


\begin{proof}
Write $m=3k+r$ for some $k\in 
\mathbb{N}$.
To show the necessity, suppose $v_{i}>$ $\floor{ \frac{m}{3}}$
for some $i$. Let $i=1$, without loss of generality.

Case 1: $r=0$

Thus, $v_{i}>$ $k$. Consider the following profile (where the first and
second lines give the preferences of individuals 1 and 2, respectively):
...
There profile admits three Pareto efficient alternatives, namely $a_{1}$, $a_{2k+1},a_{2k}$, whose
respective dispersions are $k+1$, $2k$, $k-1$. Thus, an SCR that satisfies MD must pick $a_{2k}$, which is vetoed by individual 1 \commentMN{since $a_{2k}$ is exactly the $k^{th}+1$ worst alternative of individual 1 while $v_i>k$.}.

\bigskip Case 2: $r=1$

Thus, $v_{i}>$ $k$. Consider the following profile:
...
The profile admits three Pareto efficient alternatives, namely $a_{1}$, $a_{2k+1},$ $x$, whose
respective dispersions are $k+1$, $2k+1$, $k$. Thus, an SCR that satisfies
MD must pick $x$, which is vetoed by individual 1, \commentMN{since $x$ is exactly the $k^{th}+1$ worst alternative of individual 1 while $v_i>k$.}.

\bigskip Case 3: $r=2$

Thus, $v_{i}>$ $k+1$. Consider the following profile:
...
The profile admits three Pareto efficient alternatives, namely $a_{1}$, $a_{2k+1},$ $x$, whose
respective dispersions are $k+2$, $2k+2$, $k$. Thus, an SCR that satisfies
MD must pick $x$, which is vetoed by individual 1. \commentMN{since $x$ is exactly the $k^{th}+2$ worst alternative of individual 1 while $v_i>k+1$.}.
...
\commentRS{I think the proposition above is fine. We must decide what to do with the paragraph below.}
