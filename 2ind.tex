...
\commentRS{SPEL is a defined for a given k that varies between 1 and m-1. In other words, the condition is SPEL(k) and the bigger k is the weaker the condition is. When k=1, it implies to pick among alternatives with a minimal spread. As Olivier notes, when k=m, the condition is trivially satisfied. However, as such, it is not stronger than PEL for all values of k.}
