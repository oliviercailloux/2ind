...
\textbf{Unanimity compromise} is the SCR $\FB$ that picks all alternatives with unanimous support at the minimal worst loss $\rhoP = \min_{x \in \allalts} \set{\max \lprof(x)}$; formally: $\FB(\prof) = U(\prof, \rhoP)$.
\commentOC{I may be missing the obvious, but shouldn’t we, instead of $\FB(\prof) = U(\prof, \rhoP)$, simply write $\FB(\prof) = \argmin_{x \in \allalts} \set{\max \lprof(x)}$ (and get altogether rid of $\rhoP$)? If deemed useful, we could add a small remark, for the readers who know $\FB$ by another definition, stating that this definition is equivalent to define first $\rhoP = \min_{x \in \allalts} \set{\max \lprof(x)}$ then $U(\prof, k) = \set{x \in \allalts \suchthat \max \lprof(x) ≤ k}$, and finally $\FB(\prof) = U(\prof, \rhoP)$ (essentially thus moving in this new remark the end of the current last § of the previous section).}
\begin{remark}
	\label{th:FBH}
