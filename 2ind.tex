...
%	By construction thus, $\forall a ≠ y \in A: \lprof(y)_{\ibar} < \lprof(a)_{\ibar}$, hence, $\lprof(y)_{\ibar} ≤ m - 1 - \card{(A \setminus \set{y})} = m - 1 - t$.
	...
	Turning now to the reverse direction, assume that $\khalf ≤ t < \frac{2m - 4}{3}$ and let us define a profile $\prof$ such that $\lprofinv(\intvl{0, t} × \intvl{0, t}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$.
...
\begin{corollary}
When $m = 9$, the veto rules sat MD iff they look at least to the first 6 ranks.
\end{corollary}
\begin{proof}
	$\forall t \in \intvl{4, 8}$: 
	\begin{equation}
		t ≥ \frac{14}{3} ⇔ t ≥ 5 ⇔ \forall \prof: \lprofinv(\intvl{0, t} × \intvl{0, t}) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset.
	\end{equation}
	If the PV rule has vetoes max 3, they elect every pareto within $\lprofinv(\intvl{0, t} × \intvl{0, t})$, with $t = 5$, or a larger set. Thus $\lprofinv(\intvl{0, 5} × \intvl{0, 5}) \cap \PE \subseteq f(\prof)$. One of these alternatives is in $\argmin_{\PE} (d \circ \lprof)$.
	
	If the PV rule has a veto higher than 3, say, $(0, 4)$, consider some profile st $\lprofinv(\intvl{0, 5} × \intvl{0, 5}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$. The rule is $f(\prof) = \lprofinv(\intvl{0, 8} × \intvl{0, 4}) \cap \PE$. We claim that the profile $P$ also has no alts that min disp in $f(\prof)$. Assume it has, so it has losses, say, $(6, 4)$. But an alt that minimizes dispersion not in $\lprofinv(\intvl{0, 5} × \intvl{0, 5})$ must have disp lower than this?
\end{proof}
\begin{corollary}\label{propo:pv}
