...
\end{remark}
\begin{remark}
    $\VR, \SL, \FB \in \PEcl \cap \FHcl$.
\end{remark}

We now discuss the relationship of the class $\PVcl$ to the $\FHcl$ property. To this end, we define $PV^=$ as the Pareto-and-veto rule that gives the highest equal veto power to both individuals. Thus, under $PV^=$, we have $v_1=v_2= \floor{\frac{m - 1}{2}}$, implying $v_1=v_2=\frac{(m-1)}{2}$ when $m$ is odd and $v_1=v_2= \frac{m}{2}-1$ when $m$ is even. Note that when $m$ is odd $PV^=\in\PVbcl$ while $PV^=\in\PVcl\setminus \PVbcl$ when $m$ is even.  
...
\end{proof}
A direct implication of Proposition 1 \commentRS{Can't we cite Proposition 1 automatically so that if the numbering changes the citation also changes}  is that the set of $\PVcl$ rules satisfying $FH$ is quite reduced, as made explicit by the following remark.
\begin{remark}
...
The two observations below follow Proposition 2: \commentRS{Can't we cite Proposition 2 automatically so that if the numbering changes the citation also changes} 
\begin{corollary}
...
We now establish the relationship between $\VR$, $\SL$ and $\FB$ and show that they can pick disjoint winners.
\begin{proposition}
