...
These four rules can also be compared by describing them as follows. \commentOC{Here is the description of $\SL$, which can be suitably adapted to other rules.} The $\SL$ rule is the rule that selects any alternative that is FH and that is not dominated by an FH alternative in terms of maximal welfare loses. This principle can also be described as follows. Define $\prec$ as the following partial order on the set of loss vectors: $(a, b) \prec (c, d) ⇔ \min\set{a, b} < \min\set{c, d} \land (a, b) ≤ (half, half)$. Then, $\SL$ selects $x$ iff it is an undominated element among the loss vectors that exist in $\prof$; formally, $f(\prof) = \argmin^{\preceq}_{x \in \allalts} \lprof(x)$. \commentOC{We could perhaps also describe this as an intersection of relations, one of which is common to all rules.}
