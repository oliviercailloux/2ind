...
It follows from \citet[Theorem 1]{BramsKilgour2001} that $k$-Rawlsianism is binding at every profile if and only if $k$ is at least “half”. We include a short proof for completeness.
...
\end{theorem}
\begin{proof}
	First, if $k < \khalf$, the left-hand side is false: consider the profile $\prof$ composed of an arbitrary ordering $\prefi$ over $\allalts$ and its inverse ordering $\prefiinv$ and observe that $\min \max \lprof = \khalf > k$.
	
	Second, assuming that $k ≥ \khalf$ and given any $\prof \in \allprofs$, let us show that for some $x \in \allalts$, $\max \lprof(x) ≤ k$.
