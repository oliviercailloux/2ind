...
$\PVe(\prof)=H(\prof)\cap PE(\prof)$ for each $\prof$.
\end{proposition}
\begin{proof}
At every $\prof$, $\PVe$ selects all Pareto efficient alternatives that are not among the worst $\floor{\frac{m - 1}{2}}$ alternatives for every individual. This is equivalent to say that $\PVe$ selects every Pareto efficient alternative $x$ with $\lprof(x) ≤ (\khalf, \khalf)$. Yet, this is precisely $H(\prof)\cap PE(\prof)$, as desired.
\end{proof}
...
\begin{theorem}
  \label{th:subcorr}
   	$\FB, \VR, \SL \subset \PVe$.   
\end{theorem}
\begin{proof} Proposition \ref{prop:intersection}, together with Remarks 1 and 2 establishes $\FB, \VR, \SL \subseteq \PVe$. To see the strictness of the inclusion, consider the profile
	\begin{equation}
...
\end{theorem}
\begin{proof}
	Observe that at each profile, $\bigcup(\PEcl \cap \MRcl)$ picks all Pareto alternatives, and only those, that are in the first half of both individuals’ preferences. This observation combined with Proposition \ref{prop:intersection} establishes the result.
\end{proof}
...
\commentMN{Do you mean "Among the class $\PVcl$, when $m$ is odd, only the rule $PV^=$ satisfies MR, and when $m$ is even, only the three rules $\set{\PVe, \PVv[(\frac{m}{2}, \frac{m}{2} - 1)], \PVv[(\frac{m}{2} - 1, \frac{m}{2})]}$ satisfy it." I agree, it makes the result much more attractive to read.} \commentRS{Yes, I mean that.}
...
\begin{definition}[Paretian equal loss compatibility (PEL-compatible)]
