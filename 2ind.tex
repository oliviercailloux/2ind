...
\commentRS{I made a few changes above.}
...
\end{definition}
\commentOC{Miguel Ballester observed that we’d better define the following property (name to be found). Given $k \in \intvl{0, m - 1}$, a SCR is Miguel-$k$-Rawlsian iff it prefers alternatives whose losses are within $\intvl{0, k}$ for both individuals. Formally:
	$\forall \prof \in \allprofs,  x, y \in \allalts, \lprof(x) \in \intvl{0, k}^2 \land \lprof(y) \notin \intvl{0, k}^2 ⇒ y \notin f(\prof)$.
	I believe that Miguel-$\khalf$-Rawlsianism is equivalent to $\khalf$-Ralwsianism, and Miguel-$\khalf - 1$-Rawlsianism is possibly an interesting property (that perhaps the Strong RMD rules satisfy) whereas for $k < \khalf$, $k$-Rawlsianism are uninteresting properties.
	I am in favor of making that point formally, if we can do so in a reasonably compact way.}
...
\end{proof}
\commentRS{A point that Miguel suggested: Does Theorem 14 pave the wat to show that one can always ensure a dispersion that is no more than a third of m?}
\begin{remark}
...
\commentRS{I agree.} \commentRS{A verbal explanation of the theorem below may be useful.}
\begin{theorem}
	\label{th:RMDMD}
