...
\commentOC{Define $t' = \max \set{\khalf, t}$. Do all the argument with $t'$. This shows that a profile exists where $x$ is not in the first $t'$ ranks. Hence, it is not in the first $t ≤ t'$ ranks. Doesn’t that work? Actually, it makes me realize that we can simplify the proof by just treating the case $t = \frac{2m - 5}{3}$, isn’t it?}
