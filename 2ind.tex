...
\commentOC{Drop this?} \commentRS{Why?}
...
The equal loss principle is incompatible with the one of Pareto efficiency. Indeed, consider the  profile
\begin{equation*}
        \begin{array}{ccc}
                a&b&c\\
                b&a&c\\
        \end{array}.
    \end{equation*}
    Whereas $c$ is the only alternative satisfying the equal loss principle, only $a$ and $b$ are Pareto efficient. This incompatibility is formally stated by the next theorem. 
\begin{theorem}
    $\forall m ≥ 3: \PEcl ∩ \ELcl = \emptyset$.
\end{theorem}
\commentRS{theorem 4 is missing a proof (that is rather straightforward).}\commentMN{Just added it, please check that you agree}
...
\commentOC{I will add a corollary just to have an idea.}
...
\commentRS{Here, elaborate on condition 2 of the theorem and state a lemma or remark which says that 2, 3 and 4 are pairwise equivalent. State the theorem as the equivalence between 1 and 2.}
\begin{theorem}
...
\commentOC{Add a remark to discuss this theorem verbally, and point out that 2 equiv 3 can also be seen more directly, just by anonymity.}
...
\commentRS{As Olivier noted, we have to choose whether we keep the above approach or the one below. As the impossibility is stronger with the weaker definition of MD, I am inclined to keep the weak one. To be sure, the theorem below adds something to corollary 4 (that we discussed by mail) but the this additional contribution is rather marginal and I don't think it is worth to define strong MD just for this. Thus, I suggest discarding theorem 9.}\commentMN{Let's discard Prop 9 then}
\begin{theorem}\label{propo:pv}
...
\begin{remark}
   As MD implies PEL, the class of PV rules that satisfy MD is a subclass of those that satisfy PEL. This relationship can be more precisely observed by comparing theorems \ref{propo:pel} and \ref{propo:pv}. 
