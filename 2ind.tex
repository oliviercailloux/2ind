...
\label{sec:reconc}
Given the incompatibility between MR and MD, one can attempt to reconcile the two principles by imposing minimal dispersion over the alternatives that are minimally Rawlsian. The Rawlsian minimal dispersion (RMD) principle requires the outcomes to be among the Rawlsian alternatives whose loss vectors have minimal dispersion.

\commentRS{The definition below must be strengthened so as the ensure that the rule always chooses an alternative in the first half. Theorem 15 will be true under this stronger definition, no?}
\begin{definition}[Rawlsian minimal dispersion]
	$f(\prof) \cap \argmin_{H(\prof) \cap \PE} (d \circ \lprof) ≠ \emptyset$, $\forall \prof \in \allprofs$.
\end{definition}
...
By Theorem 4, the choice of $\PVe$ consists of all Pareto efficient alternatives in $H(\prof)$, thus including the alternative that minimizes the loss dispersion.
\end{proof}

COMMENTS FROM RS: The reference above to Theorem 4 should be made automatic.
Here, we define the RMD rule that only pick first half alternatives that minimize dispersion and compare it to FB.
