...
\end{proposition}
\commentRS{Prove the proposition and compare it with Theorem 2 and Proposition 5.}
...
A tradeoff function is $t(a, b) \in \N \cup (\N + \frac{1}{2})$ where $a$ is the ineq level and $b$ is the delta min, and it indicates the least delta ineq improvement acceptable for that min worsening. 

A tradeoff rule is $f^t$, parameterized by a tradeoff function, defined as $f^t(\prof)$ selecting maximal elements according to losses $(min, ineq)$ according to the preference as determined by the tradeoff function. More precisely, $(m, i)$ is preferred to $(m', i')$ iff loss min is not compensated enough by gain ineq: $i - i' < t(i', m' - m)$. Note that $i - i'$ is the gain ineq; $m' - m$ is the loss min; $t(i, m' - m)$ is the least delta ineq acceptable for that min worsening.

\begin{example}
$t(5, 1) = 2$ indicates that at ineq 5, a worsening of 1 min (thus, plus 1 min) is acceptable in exchange of an improvement of at least 2 ineq (thus, minus 2 ineq) (or 3 ineq, thus, minus 3 ineq, …), thus, $(1, 5) ≤ (2, 3)$ (and, of course, $(1, 5) ≤ (2, 2)$).
Also, $t(3, -1) = -2$ indicates that at ineq 3, an improvement of 1 min (thus, minus 1 min) is acceptable in exchange of a worsening of at most 2 ineq (thus, plus 2 ineq) (or 1 ineq, thus, plus 1 ineq, …), means we want a worsening of at least 2 ineq (thus, plus 2 ineq, or plus 3 ineq, …): $(1, 4) ≥ (1, 5) ≥ (2, 3)$.

Given $t(5, 1) = 1.5$, with $(m, i) = (1, 5)$ and $(m', i') = (2, 3)$: $(1, 5)$ preferred to $(2, b)$ iff $5 - b < 1.5$ (that is, $b > 3.5$), thus, $(2, 4) ≤ (1, 5) ≤ (2, 3) ≤ (2, 2)$.
%Equivalently, given $t(3, -1) = -1.5$, with $(m, i) = (1, 5)$ and $(m', i') = (2, 3)$: $(1, b)$ preferred to $(2, 3)$ iff $b - 3 < $(2, 3) ≥ (1, 5)$.
%$(m, i) = (2, 3)$ and $(m', i') = (1, 5)$: 
\end{example}
...
\begin{conjecture}
	LVC and shifting does not imply w-c.
\end{conjecture}
\begin{proof}
	Assume that $x$ with losses $(2, 5)$ is preferred (according to $\succeq$) to $y$ with losses $(1, 6)$.
	...
	Not guaranteed, it seems: I could have $(1, 4) > (0, 5) > (0, 6) > (1, 5)$.
\end{proof}
