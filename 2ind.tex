...
\commentOC{I find \cref{rk:variant} possibly enlightening but I admit that this may be overwhelming for the first reading. So if Remzi is opposed and Matías is not in favor of keeping it, we can delete it. Another possibility is to transform it into a footnote to make it clearer that it can be skipped on first read (though I think that the clear demarcation as a Remark conveys the “can-be-skipped” information equally well).}
...
%	\commentOC{It would be more elegant to show that any other alternative than $y$ has either $\max\lprof(x) ≤ t$ or $d(x) ≥ d(y)$.}
	...
	\commentOC{There seems to be a problem here: we don’t have that $t ≥ \khalf$. I don’t remember what I had in mind when writing this. Do you see a way of saving this argument?}
