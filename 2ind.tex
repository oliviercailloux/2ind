...
\end{proof}
\commentOC{This proof seems perhaps too complex compared to the relative intuitiveness of this result but I can’t see a simpler one for now. Any idea?} \commentRS{I think that is fine.}
...
\begin{proof}
	...
	Name the alternatives $a_1, …, a_{\lprof(a)_i}, b_1, …, b_{\lprof(a)_i}, x, y, c_1, …, c_{m - 2 \lprof(a)_i - 2}$. 
	Define the sequences $A_1 = (a_1, …, a_{\lprof(a)_{\ibar}})$, $A_2 = (a_{\lprof(a)_{\ibar} + 1}, …, a_{\lprof(a)_i})$, $B_1 = (b_1, …, b_{\lprof(a)_{\ibar}})$, $B_2 = (b_{\lprof(a)_{\ibar} + 1}, …, b_{\lprof(a)_i})$, $C = (c_1, …, c_{m - 2 \lprof(a)_i - 2})$ and the following profile $Q$:
	\begin{equation}
		\begin{array}{*{25}{l}}
		...
		\end{array}.
	\end{equation}
	We see that $f^\succeq(Q) \subseteq \set{x, y}$, by Strong RMD: $\forall c \in C, c \notin \PD$ and $\forall z \in A_1 \cup A_2 \cup B_1 \cup B_2, \max \lprof(z) ≥ \lprof(a)_i + 2 + m - 2 \lprof(a)_i - 2 = m - \lprof(a)_i = \khalf + \floor{\frac{m - 1}{2}} + 1 - \lprof(a)_i$ and with $\lprof(a)_i ≤ \khalf$, $\max \lprof(z) ≥ \floor{\frac{m - 1}{2}} + 1$, thus $z \notin H(Q)$.
	...
	Assume that ${<} \subseteq {\prec}$. (This should be established.)
	...
	To be done…
%$a \in \argmin s$ and let’s show that $a \in f^\succeq(\prof)$.
	
%A LVC rule $f^\succeq$ that is Pareto must have $\forall \prof, a: \lprof(a) < \lprof(a') => \lprof(a) \prec \lprof(a')$.
%
%Consider any $\prof, a$ such that $\lprof(a) < \lprof(a')$. 
%	Indeed, $f^\succeq$ is Pareto $⇔ \forall \prof: f(\prof) \subseteq \PE$. $a in fp <=> forall a': l(a) <= l(a')$. $fp = argmin ≤ o lp$
%Thus $argmin ≤ o lp \subseteq \PE$.
%Now $lp(argmin ≤ o lp) = min ≤ o lp$.
%As lp is a bijection, equiv, $lp(argmin ≤ o lp) \subseteq lp\PE$.
%Thus $min ≤ o lp \subseteq lp \PE$.
%We obtained $f^\succeq$ is Pareto $⇔ \forall \prof: min ≤ o lp \subseteq lp \PE$.

%min_R (<) = 0 = {r st <(r) = R \ r} = min <|R.
%min_X f = 0
%min_X < o f = 0 = min <|f(X)
%min_R ≤ = ≤-1(R) = min ≤|R
%argmin_X f = x1 st f(x1) = 0 thus f-1(0) if its in X, otherwise f-1(…)
%argmin_X < o f = x1
%NB: f(argmin_X < o f) = min_X < o f.
%
%f-1(x1) = 0
%f-1(argmin_X < o f) = min_X < o f = min < | f(X).
%indeed, r in f-1(A) => 

%$\forall \succeq$, [$f^\succeq$ satisfies Pareto] iff [$\forall l: l ≤ l' ⇒ l \preceq l'$].
\end{proof}
