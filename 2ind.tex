...
\commentRS{The proof we suggested yesterday for Propostion 11 is almost true. More precisely, it holds when m is a multiple of 3 or a multiple of 3 modulo 1. However, it fails for modulo 2. I had to change the statement of the proposition as below.}

\begin{proposition}\label{propo:pv}
	Take any $m ≥ 3$ and write $m=3k+r$ for $k\in \mathbb{N}$, $r\in\{0,1,2\}$.
	(i) For $r\in\{0,1\}$, a PV rule $\PVv[(v_1, v_2)]$ satisfies MD iff \max_{i \in \set{1, 2}} v_i ≤ $k$.
	(ii) For $r=2$, a PV rule $\PVv[(v_1, v_2)]$ satisfies MD only if \max_{i \in \set{1, 2}} v_i ≤ $k+1$.
		(iii) For $r=2$, a PV rule $\PVv[(v_1, v_2)]$ satisfies MD if \max_{i \in \set{1, 2}} v_i ≤ $k$.
\end{proposition}
