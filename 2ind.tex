...
\begin{lemma}
	\label{th:PEmpty}
	Given $m ≥ 2$, 
	$\exists \prof \suchthat \lprofinv(\intvl{0, \ceil{\frac{2m - 7}{3}}} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$. Equivalently, given $t \in \N$, $t < \frac{2m - 4}{3} ⇒ \exists \prof \suchthat \lprofinv(\intvl{0, t} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$.
	Similarly,
	$\exists \prof \suchthat \lprofinv(\intvl{0, m - 1} × \intvl{0, \ceil{\frac{2m - 7}{3}}}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$
	and
	$t < \frac{2m - 4}{3} ⇒ \exists \prof \suchthat \lprofinv(\intvl{0, m - 1} × \intvl{0, t}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$.
\end{lemma}
\begin{proof}
	We prove only the first two sentences of the claim, the other one admits a similar proof swapping the roles of individuals 1 and 2.
	...
	we will define a profile $\prof$ such that $\lprofinv(\intvl{0, t} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$, which implies that $\lprofinv(\intvl{0, \ceil{\frac{2m - 7}{3}}} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$.
	...
	The second sentence of the claim is equivalent because $\forall t \in \N$, $t < \frac{2m - 4}{3} ⇔ t ≤ \ceil{\frac{2m - 7}{3}}$: if $\frac{2m - 4}{3} \in \N$, $t < \frac{2m - 4}{3} = \ceil{\frac{2m - 4}{3}} ⇔ t ≤ \ceil{\frac{2m - 4}{3}} - 1$; and otherwise, for some $0 < ε < 1$, $t < \frac{2m - 4}{3} = \ceil{\frac{2m - 4}{3}} - ε ⇔ t ≤ \ceil{\frac{2m - 4}{3}} - 1$.
\end{proof}
