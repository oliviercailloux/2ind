...
\commentOC{I suggest to replace $\rhoP$ in the last sentence with $\min_{x \in \allalts} \set{\max \lprof(x)}$, which I believe will help the reader see the link between the statement and the proposition.}
\end{remark}
...
\end{remark}
\commentOC{Can we integrate this remark into the statement of the theorem, by adding: “and $\FB \cup \VR \cup \SL \subset \PVe$”?}
...
\end{proof} 
\commentRS{I think the observation made by the remark above must be the statement of the theorem.}
\commentOC{I have rephrased accordingly.}
...
\commentOC{Then the EL in the following proposition should be changed to ELC, shouldn’t it?}
...
\commentOC{I agree that “MD compatibility” is more coherent, but it is not a nice name (and impractical to use). I favor sticking to MD as we do not need the stronger one, but I do not object strongly if you prefer it otherwise.}
...
\commentOC{We refer to these lemmas in several proofs of theorems that will not go to the appendix. Is this a problem?}
\begin{lemma}
	\label{th:PNonEmpty}
...
\commentOC{I had in mind that we agreed to use what is now in a comment at the start of this article as the “selling argument” for the article. Thus, that we stick to five theorems that should correspond to the story that the selling argument describes. Do we want to adopt another approach instead?}
