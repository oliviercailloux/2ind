...
More resoluteness can be achieved by introducing the notion of \emph{anonymous dominance} of losses. Given two losses $l, l' \in \alllosses$, say that 
%$l$ weakly dominates $l'$ iff $\max l ≤ \min l'$ and $l$ strictly dominates $l'$, denoted $l ⊲ l'$, iff $l$ weakly dominates $l'$ and $l'$ does not weakly dominate $l$. 
$l$ dominates $l'$, denoted $l ⊲ l'$, iff $\max l ≤ \min l'$ and $\max l' > \min l$.
Given a profile $\prof$, let $E(\prof)$ denote the alternatives whose losses are not dominated: $E(\prof) = \set{x \in \allalts \suchthat \forall y \in \allalts: \lprof(y) \not⊲ \lprof(x)}$.
We can then define the \emph{resolute RMD} rule as $\argmin_{H(\prof) \cap \PE \cap E(\prof)} (d \circ \lprof)$.
Note also that this tie breaking mechanism is equivalent to minimize the sum of losses (equivalently, pick the Borda preferred alternative), within the set of alternatives that have minimum spread: given two losses $l, l'$ with $d(l) = d(l')$, $l$ dominates $l'$ iff $\sum l < \sum l'$.
