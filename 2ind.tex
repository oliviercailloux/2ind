...
\begin{theorem}{(\citeauthor{BramsKilgour2001})}
	\label{th:BKkhalf}
...
\commentMN{I made several cosmetic changes to the proof of $1 \Longrightarrow 2$, please check that you agree}
\begin{proof}
	For the claim $\ref{it:tbound} ⇒ 2$,  consider any $t ≥ \frac{2m - 4}{3}$ (\commentMN{$t ≥ \frac{2m - 5}{3}$ } ).  We now show that the set $\lprofinv(\intvl{0, t} × \intvl{0, t}) \cap \argmin_{\PE} (d \circ \lprof)$ is non-empty for any profile $\prof$. Consider any $x \in \argmin_{\PE} (d \circ \lprof)$, and let $i$ denote any individual such that $\max \lprof(x) = \lprof(x)_i$. 
 ...
 If $\lprof(x)_i > t$, we let $A = \set{a \suchthat \lprof(a)_i ≤ t}$ be the $t + 1$ top alternatives for $i$. 
	...
	It follows that $\lprof(x)_{\ibar}<\lprof(y)_{\ibar}  $ to avoid that $y$ Pareto-dominates $x$. Moreover, if $\lprof(y)_{\ibar} \leq \lprof(y)_i $, then $$\lprof(x)_{\ibar} < \lprof(y)_{\ibar} ≤ \lprof(y)_i < \lprof(x)_i$$ which implies that $d(\lprof(y)) < d(\lprof(x))$, contradicting $x \in \argmin_{\PE} (d \circ \lprof)$. Thus, we have $\lprof(y)_i < \lprof(y)_{\ibar}$.
	 ...
	 Observe that as $t \in \N$, $t ≥ \frac{2m - 4}{3} ⇒ t ≥ \ceil{\frac{2m - 4}{3}}$ ( (\commentMN{$t ≥ \frac{2m - 5}{3}⇒ t ≥ \ceil{\frac{2m - 5}{3}}$ } ), and when $m ≥ 3$ \commentMN{$m\geq 5$}, $\ceil{\frac{2m - 4}{3}} ≥ \khalfAlt$ ( \commentMN{$\ceil{\frac{2m - 5}{3}} ≥ \khalfAlt$}); whence $t ≥ \khalfAlt$.%
	 \footnote{(\commentMN{If $m ≥ 7 ⇒ \frac{2m - 5}{3} ≥ \frac{m - 1}{2}$ so that the result directly follows. If $m=5$, $\ceil{\frac{2m - 5}{3}}=\ceil{\frac{10}{3}}=3$ and $\khalfAlt=\ceil{\frac{4}{2}}=2$ and if $m=6$, $\ceil{\frac{2m - 5}{3}}=\ceil{\frac{7}{3}}=3$ and $\khalfAlt=\ceil{\frac{5}{2}}=3$. With $m=4$, the result is not valid since  $\ceil{\frac{2m - 5}{3}}=\ceil{\frac{3}{3}}=1$ while $\khalfAlt=\ceil{\frac{3}{2}}=2$}
  
  
  When $m$ is odd, $m ≥ 5 ⇒ \frac{2m - 4}{3} ≥ \frac{m - 1}{2} = \khalfAlt$ and when $m$ is even, $m ≥ 8 ⇒ \frac{2m - 4}{3} ≥ \frac{m}{2} = \khalfAlt$; when $m = 3$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{2}{3}} = 1 = \khalfAlt$; when $m = 4$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{4}{3}} = 2 = \khalfAlt$; when $m = 6$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{8}{3}} = 3 = \khalfAlt$.} 
	
	By construction, $\forall a ≠ y \in A: \lprof(y)_{\ibar} < \lprof(a)_{\ibar}$, hence, $\lprof(y)_{\ibar} ≤ m - 1 - \card{(A \setminus \set{y})} = m - t - 1$.
	Also, $t ≥ \khalfAlt$ thus $m - t - 1 ≤ m - 1 - \khalfAlt = \floor{\frac{m - 1}{2}} ≤ t$.
	These two facts, together with $\lprof(y)_i < \lprof(y)_{\ibar}$, establish that $\lprof(y)_i < \lprof(y)_{\ibar}≤ m - t - 1 ≤ t$, which yields that $y \in \lprofinv(\intvl{0, t} × \intvl{0, t})$ and that $  m - t - 1 \geq d(\lprof(y))$. 
 ...
 $$t ≥ \frac{2m - 4}{3} \Longleftrightarrow t + 1 ≥ \frac{2m - 1}{3} \Longleftrightarrow 2 (t + 1) - m + 1 ≥ m - t - 1,$$ 

 \commentMN{$$t ≥ \frac{2m -5}{3} \Longleftrightarrow t + 1 ≥ \frac{2m - 2}{3} \Longleftrightarrow 2 (t + 1) - m + 1 ≥ m - t - 2,$$ }

 \commentMN{Not sure how to continue here since it seems that we really need $\frac{2m-4}{3}$}
 
 and hence $d(\lprof(x)) ≥ m - t - 1 ≥ d(\lprof(y))$, which implies that $y \in \argmin_{\PE} (d \circ \lprof)$.
	...
	\commentOC{There seems to be a problem here: we don’t have that $t ≥ \khalf$. I don’t remember what I had in mind when writing this. Do you see a way of saving this argument?} \commentRS{OMG.} \commentMN{OMG!!!}
\commentOC{Define $t' = \max \set{\khalf, t}$. Do all the argument with $t'$. This shows that a profile exists where $x$ is not in the first $t'$ ranks. Hence, it is not in the first $t ≤ t'$ ranks. Doesn’t that work? Actually, it makes me realize that we can simplify the proof by just treating the case $t = \frac{2m - 5}{3}$, isn’t it?} \commentMN{Do you mean that the equivalence 1-2-3-4 hold when $t\geq (2m-5)/3$? I was following the argument and it makes sense.}
...
For the latter, consider the profile $\prof$ with $m=7$:
	...
	\begin{equation}
		...
		\end{array}.
	\end{equation}
Observe that $PV^{(2, 2)}(\prof)=\{c,d,e\}$  whereas $\argmin_{\PE} (d \circ \lprof) = \set{d}$.
...
\begin{proof}
	...
	\commentOC{We actually need to show that some PE alt is such that $\max \lprof(x) ≤ k$, which can of course easily be argued here, but I am pretty sure this argument is already somewhere in the paper, but where? I can’t find it any more now from a cursory look.}
...
It also follows from \cref{th:equal} that $\PVe$ fails Strong RMD. Thus, no rule that we have examined so far satisfies Strong RMD.
\commentOC{We might want to choose the name once we understand the rule a bit better, perhaps.} \commentRS{Yes. We now also mention the rule at the introduction.}
...
We can then define the \emph{resolute RMD} rule as $\argmin_{H(\prof) \cap \PE \cap E(\prof)} (d \circ \lprof)$.
