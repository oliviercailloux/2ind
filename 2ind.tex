...
\begin{proof}
	...
	Define the (possibly empty) sequences $A_1 = (a_1, …, a_{l(i)})$, $A_2 = (a_{l(i) + 1}, …, a_{\max l' - 1})$, $B_1 = (b_1, …, b_{\min l})$, $B_2 = (b_{\min l + 1}, …, b_{\max l' - 1})$, $C = (c_1, …, c_{m - 2 \max l'})$, and the profile $\prof$ defined as:
	\begin{equation}
		\begin{array}{*{25}{l}}
		...
		\end{array}.
	\end{equation}
	At $\ibar$ we put $y$ before $x$ iff $\min l = \min l'$.
	Choose $\ibar$ so that $B_1$ ends at $\min l$ iff $\min l = \min l'$, at $\min l'$ otherwise.
	Thus $B_1$ ends at $\max {\min l, \min l'}$.
