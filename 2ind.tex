...
\commentRS{SPEL is a defined for a given k that varies between 1 and m-1. In other words, the condition is SPEL(k) and the bigger k is the weaker the condition is. When k=1, it implies to pick among alternatives with a minimal spread. As Olivier notes, when k=m, the condition is trivially satisfied. Thus, SPEL is not stronger than PEL for all values of k.}
...
PV, FB, SL and VR, all fail strong PEL. \commentRS{This seems to be true for values of k that are between 1 and m/2.}

In fact, strong PEL and FH are incompatible conditions. \commentRS{Again this seems to be true for values of k that are between 1 and m/2.}

\commentRS{I wonder whether we should drop SPEL and simply use the minimal spread condition.}
