...
$k$-Rawlsianism is binding at exactly those profiles where some alternative $x$ has its losses in $\intvl{0, k}^N$ (equivalently, satisfies $\max \lprof(x) ≤ k$), and is void at the other profiles. It is thus natural to wonder which values of $k$ make the constraint binding at every profile, so as to guarantee a minimal possible loss to every individual  whatever the profile.\footnote{%
	\label{rk:variant}
...
\commentRS{Choosing k as the "true" half is equivalent to our "artificial" half under SRMD. I find this worthy to remark and its place is, naturally, in the section where we define SRMD.} 
Strong RMD implies MR, equivalently, it implies $\khalf$-rawlsianism. It also implies $\floor{\frac{m - 1}{2}}$-rawlsianism \commentOC{Does it?}.
%\begin{theorem}[false?]
%	For any rule $f$ satisfying Strong RMD,
%	$\forall \prof \in \allprofs,  x, y \in \allalts, \max \lprof(x) ≤ \khalf - 1 \land \max \lprof(y) > \khalf - 1 ⇒ y \notin f(\prof)$.
%\end{theorem}
%\begin{proof}
%	\commentOC{This seems false. With $m = 7$, thus $\khalf = 3$ and $\khalf - 1 = 2$, consider $(abcdefg)$ for individual $1$ and $(cefbadg)$ for individual $2$ and $f$ picking $b$.}
%Pick prof that has some $x$ with $\max \lprof(x) ≤ \khalf - 1$. Is it possible that $f$ picks some $y$ with $\khalf - 1 < \max \lprof(y) ≤ \khalf$? Thus, $\max \lprof(y) = \khalf$? Thus $x ≠ y$.
%Pareto of $y$ requires that for some $i$, $\lprof(y)_i < \lprof(x)_i$. As $\forall i: \lprof(x)_i ≤ \max \lprof(x) ≤ \khalf - 1$, $i \suchthat \lprof(y)_i < \lprof(x)_i ⇒ i \notin \argmax \lprof(y)$. Thus, $\min \lprof(y) < \min \lprof(x) ≤ \max \lprof(x) < \max \lprof(y)$ but then $y$ is not min spread.
%\end{proof}
\begin{theorem}[false?]
	For any rule $f$ satisfying Strong RMD,
	$\forall \prof \in \allprofs,  x, y \in \allalts: \max \lprof(x) ≤ \floor{\frac{m - 1}{2}} < \max \lprof(y) ⇒ y \notin f(\prof)$.
\end{theorem}
\begin{proof}
	\commentOC{This seems false. With $m = 8$, thus $\khalf = 4$ and $\floor{\frac{m - 1}{2}} = 3$, consider $(abcdefgh)$ for individual $1$ and $(hdfgcbea)$ for individual $2$ and $f$ picking $c$, forbidden by $\floor{\frac{m - 1}{2}}$-rawlsianism because of $d$.}
\end{proof}
