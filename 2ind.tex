...
\Citet{cailloux2022compromising} propose a different conception of compromising based on an \textit{equal loss principle} that favors outcomes where every individual concedes as equally as possible from his highest ranked alternative. They show that several two-person social choice rules fail one variant of it \commentOC{This sentence feels odd. Can we write “… fail this principle”? Or “… fail this principle or some variant of it”? Or “… fail it”?}.
...
\commentOC{I believe that we should start by proving that all $\PVv$ rules are different.}
\begin{remark}
...
To see the equivalence with the statement of \cref{th:pvmr} \commentOC{for $m$ even}, note that $\forall i \in \set{1, 2}, v_i ≥ \floor{\frac{m - 1}{2}}$ is equivalent to $\abs{v_1 - v_2} ≤ 1$.
\commentOC{I find this quite difficult to understand. Could we move the interpretation after the theorem and proof?}
\begin{theorem}
	\label{th:pvmr}
...
\commentOC{Seems to me that we’re missing a sentence here that helps the reader understand the role of the following theorem. Something like: The next theorem will permit to establish the relationship of MD wrt the other classes of rules considered in this article.}
...
\commentMN{Je trouve que la regle $MD$ est vraiment bien. Elle me semble vraiment une règle intéressante. Elle est bien définie, Pareto efficace. Est elle anonyme et neutre? Peux t'on dire d'autres choses? FB ne coïncide pas avec $MD $ car aucune règle qui satisfait $MR$ le fait. Olivier avait trouvé un exemple astucieux avec 7 alternatives ou plus où l'on voyait pourquoi. Peut-on le rajouter? Je trouve que c'est surement la sous-correspondence plus intéressante de $PV^=$ avec FB. Doit on parler de $MD$ dans l'intro? }

\commentRS{MD is anonymous and neutral. It is not Maskin monotonic. What other properties it satisfies? Looking at the properties of the equal are rule of Thomson may inspire. By the way, in our paper with Beatrice, the minmal dispersion idea was inspired by the concept of equal losses (for continuous utilities) used for bargaining (Chun
1988; Chun and Peters 1991) and bankruptcy problems (Herrero and Villar 2001). If we wish to further analyze the rule MD we may need to look at these papers as well. I think we have to decide how comprehensive we wish to talk about the rule MD}
