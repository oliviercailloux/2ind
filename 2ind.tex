...
\label{sec:intro}
\commentOC{Someone suggests to rename the title to: Two-person compromises}
...
CONJECTURE: A PV rule satisfies MD iff the rule endows no individual with a veto power that exceeds 1/3 of the alternatives.
\commentRS{Compare this conjecture with Proposition 5.}
...
\begin{theorem}[Under w-c, SPEL equivalent to care for equality, at once]
	\label{th:spelEquiv}
...
\begin{proof}
	...
	\begin{equation}
		\forall y ≠ x \in \PE: [-w_m \min \lprof(y) - w_i (\max \lprof(y) - \min \lprof(y)) < -w_m k],
	\end{equation}
	itself equivalent to
	\begin{equation}
		\label{eq:wNotInF}
		\forall y ≠ x \in \PE: [w_m (k - \min \lprof(y)) < w_i (\max \lprof(y) - \min \lprof(y))].
	\end{equation}
	To conclude, we show that $w_m ≤ w_i$ implies \eqref{eq:wNotInF} and that \eqref{eq:wNotInF} implies $w_m ≤ w_i$.
	
	Observe first that $\forall y ≠ x \in \PE: k < \max \lprof(y)$ (otherwise, $y$ is Pareto-dominated by $x$); equivalently, $k - \min \lprof(y) < \max \lprof(y) - \min \lprof(y)$. Thus, $m_m ≤ w_i$ implies \eqref{eq:wNotInF}.
	
	Second, considering any $\prof$ satisfying \eqref{eq:bigIneq} and considering $k = \frac{m - 1}{2}$, the condition implies that $w_m (\frac{m - 1}{2}) < w_i (\frac{m + 1}{2})$, equivalently, $w_m < w_i \frac{m + 1}{m - 1}$, and because this must be true for all $m$, this implies $w_m ≤ w_i$.
\end{proof}
...
\begin{proof}
	...
	Observe that $\lprof[\prof^k](x) = (k, k)$ and $\lprof[\prof^k](a_0) = (1, k + 1)$ \commentOC{pbl here}.
...
\begin{proof}
	...
	\begin{align*}
		...
		&⇔ w_m (k - \min \lprof(y)) \geq w_i (\max \lprof(y) - \min \lprof(y)) \\
		&⇔ k - \min \lprof(y) \geq  \max \lprof(y) - \min \lprof(y) \: (\:\text{since } w_i \geq w_m \:)\\
		&⇔ k \geq  \max \lprof(y).
	\end{align*}
...
\begin{remark}
	The weight-comparing property could perhaps be turned into more behavioral axioms with the help of something like Shifting (see \cref{def:shifting}). I suspect that there is a classical trick to do this.
