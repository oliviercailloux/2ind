...
Note that $[\forall i: v_i ≥ \floor{\frac{m - 1}{2}} \land \sum_i v_i ≤ m - 1]$ is equivalent to $[\exists i \suchthat v_i = \floor{\frac{m - 1}{2}} \land \floor{\frac{m - 1}{2}} ≤ v_{\ibar} ≤ \khalf]$. Thus, \cref{th:pvmr} implies that among the class $\PVcl$, when $m$ is odd, only the rule $PV^=$ satisfies MR, and when $m$ is even, only the three rules $\set{\PVe, \PVv[(\frac{m}{2}, \frac{m}{2} - 1)], \PVv[(\frac{m}{2} - 1, \frac{m}{2})]}$ satisfy it.
...
Given a profile, let us call “RMD alternatives” the alternatives that minimize dispersion among the paretian alternatives within the first half.
\commentOC{I include here a remark of Remzi just to check that I interpreted it correctly. It can be deleted afterwards.}
\commentRS{we can define a stronger version of it (this is not the alternative condition in the paper we already discussed) that requires the social choice to always contain an RMD alternative and to remain in the first half. Of course, this stronger version of the RMD condition satisfies MR by direct definition (while it fails MD). Moreover,  our Theorem 15 seems to hold under this version.
I will be happy to keep this new version of the RMD condition because it has the same format as the other equal loss conditions we considered, namely PEL, SPEL, MD. All these conditions require a particular “equal loss alternative” to be within the social choice set without asking this choice set to consist only of equal loss alternatives. Thus, I propose to replace the old version of the RMD condition with this new one and keep Theorem 15 as it is (hoping that I am not wring in saying that Theorem 15 holds under the new version).
 However let’s not stop here. The strongest version of the RMD condition that requires the social choice set to be a subset of RMD alternatives is promising. (This is the one we already discussed.) In fact, this looks more like a social choice rule than a condition and it may worth exploring its properties.}
The Rawlsian minimal dispersion (RMD) principle requires the social choice to always contain an RMD alternative and to remain in the first half.
...
\begin{remark}
	RMD strengthens MR (thus, Strong RMD as well). Also, Strong RMD (thus, RMD) is not stronger than MD as a paretian strict minimizer of dispersion might be out of the first half. Consider for example $\prof = \set{(1, (a, b, c, d, e, f)), (2, (d, f, e, a, b, c))}$ (where the “first half” alternatives are those with both losses not worst than 3, thus $\set{a, d}$): satisfying MD requires to pick $e$ which strictly minimizes dispersion and is Paretian, while a Strong RMD rule must pick within $\set{a, d}$.
	\commentOC{I might be interesting to prove that $\forall m$, Strong MD is not stronger than MD}.
\end{remark}

\begin{theorem}
     $\FB$, $\VR$, $\SL$ and all rules in $\PVcl$ except $\PVe$ fail RMD.
     \commentOC{I find it slightly ambiguous whether the theorem also claims that $\PVe$ satisfies RMD, or only that the rules in PV that are not $\PVe$ do fail it (we want the first interpretation). I suggest the phrasing “$(\set{\FB, \VR, \SL} \cup \PVcl) \cap \RMDcl = \set{\PVe}$”, but I will not complain if you find this not very readable. A perhaps less unreasonably pedantic option is “$\FB$, $\VR$, $\SL$ all fail RMD, and $\PVcl \cap \RMDcl = \set{\PVe}$.”}
\end{theorem}
\begin{proof}
...
\begin{conjecture}
	The Strong RMD rule selects at most two alternatives.
\end{conjecture}
