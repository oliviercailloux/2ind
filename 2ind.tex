...
In concordance with the ceiling established by Theorem 1 of \cite{BramsKilgour2001}, we use the term “half” to mean the smallest integer $k$ that exceeds $m-k$ \commentOC{to \emph{refer to} the smallest integer $k$ that \emph{strictly} exceeds $m - k$?} \commentRS{I don't think this is needed as there is no such thing as "weakly exceeding".}\commentMN{agree with Remzi} and for every profile $\prof \in \allprofs$, we let $H(\prof) = \set{x \in \allalts \suchthat \lprof(x) ≤ (\ceil{\frac{m - 1}{2}}, \ceil{\frac{m - 1}{2}})}$ denote the set of alternatives reaching the best half of every individual’s preference. Given $\prof$ and a loss level $k \in \losses$, define $U(\prof, k) = \set{x \in \allalts \suchthat \lprof(x) ≤ (k, k)}$ as the set of alternatives imposing losses not higher than $k$ for all individuals. 
...
\commentRS{I made the changes proposed by Olivier and agreed by Matias. A question: why we say k-bounded? What kind of a bound we mean.}

\begin{definition}[$k$-bounded Rawlsianism] Given $k \in \intvl{0, m - 1}$, a SCR is $k$-bounded Rawlsian iff it selects its winners among those alternatives whose losses are within $\intvl{0, k}$ for both voters. Formally:
	$\forall \prof \in \allprofs,  f(\prof) \subseteq \lprofinv(\intvl{0, k} × \intvl{0, k})$.
\end{definition}
As noted in the introduction, $k$-Bounded Rawlsianism can be satisfied only when $k$ is at least “half”.
\begin{proposition}
	Given $k  < \khalf$, no rule is $k$-bounded Rawlsian.
\end{proposition}
We define “minimal Rawlsianism (MR)” through the minimal $k$ for which $k$-Bounded Rawlsianism is satisfiable. Hence, being “minimally Rawlsian (MR)” is identical to $\khalf$-Bounded Rawlsianism. 
\begin{definition}[Minimal Rawlsianism (MR)] A rule f satisfies MR if 
	$\forall \prof \in \allprofs,  f(\prof) \subseteq H(\prof)$.
\end{definition}

Let $\mathcal{MR}$ denote the class of minimally Rawlsian (MR) rules.
...
\commentRS{May bad. I wrote "Proposition 7 but I meant Proposition 8. I hadn't touch the proof of Proposition 7, which I find fine, except that the set of alternatives is called m.}\commentMN{I corrected $m$ being the set of alternatives in the proof of Prop 7. I also meant Prop 8 ..}
...
Note that $d \in \Sigma$ coincides with multiple commonly used spread measures, as shown in \cref{sec:spreads} \commentOC{Appendix? Footnote? Remark? To be discussed}. \commentRS{Certainly not "appendix". I think a footnote is fine but I would be happy with a remark as well.}
...
\begin{proof}
	...
	If $k > \khalf = \frac{m}{2}$, equivalently, $k ≥ \frac{m}{2} + 1$ or $m ≤ 2k - 2$, then $\lprof(x)_{\ibar} ≤ k - 3$. Thus the dispersion of $x$ is at least $3$. This implies that another alternative in first half has a dispersion of at most 3, as it is impossible to place the $\frac{m}{2}$ first alternatives for voter $i$ beyond $x$ for $\ibar$ without any of those having a dispersion of 3 or less. \commentOC{End of the argument to be corrected.}
\end{proof}
	
\commentRS{I will read the proofs of Theorem 3 and Proposition 9.}The minimal Rawlsian and minimal dispersion principles being logically incompatible, the rules that satisfy MR (namely FB, VR, SL and the PV rules identified by Theorem 2) all fail MD. As Theorem 2 shows that most PV rules fail MR, the following result which determines the class of PV rules satisfying MD is of interest. 
...
\end{proposition}
\commentRS{I'll send a hand-written sketch of the proof.}
...
\commentRS{I worked on the proof below but its latex needs to be corrected and the end must be completed.}

\commentMN{Thanks for the nice work, I updated Tex and the end of the proof as suggested.}
...
$H(\mathbf{P})=\{x, y\}$ and for a rule $f$ to satisfy $RMD$, we must have $y\in   f(\mathbf{P})$. However, only alternatives $x$ and $y$ are in the first half. $RMD$ requires $y$ to be among the picked alternatives. Both $FB(\mathbf{P})$ = $VR(\mathbf{P})=\{x\}$ thus both rules fail $RMD$. 
...
Thus, by Theorem 2, $\PVe$ is the only candidate to satisfy $RMD$. Indeed, when $m$ is even, $\PVe$ is the only $PV$ rule that selects only alternatives in $H(\mathbf{P})$. When $m$ is odd, $\PVe$ is the only $PV$ rule that selects only alternatives in $H(\mathbf{P})$ (it suffices to consider perfectly opposed preferences). Moreover, $\PVe$ chooses all Pareto efficient alternatives in $H(\mathbf{P})$. Thus, it includes for all profiles, the alternative which minimizes the loss dispersion, proving the claim. \commentMN{Much better, thanks}
\end{proof}



A rule that satisfies this principle is the rule that selects among the MR alternatives exclusively the ones with the minimal dispersion so that, formally, $ \arg min_{H(P)} (d \circ \lprof)= f^{*}(\mathbf{P})$, $\forall \prof \in \allprofs$. The rule $f^*$ satisfies $MR$ and is hence logically incompatible with MD. 
