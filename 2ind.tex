...
It should also be noted that in a recent paper, \citet{barbera2022compromising} use the term unanimity compromise in a framework with no disagreement point to refer to fallback bargaining in the sense of \citet{BramsKilgour2001}.
...
\begin{itemize}
	...
	\item The \textit{veto-rank rule} where, given an odd number $m$ of alternatives, each individual vetoes $(m - 1) / 2$ alternatives and ranks the remaining $(m+1) / 2$. The outcome is the alternatives with the minimal sum of ranks among those that have not been vetoed.
	\item The \textit{shortlisting rule} where one individual selects her best $(m+1) / 2$ alternatives and the rule picks the best alternative of the other individual out of that shortlist.
...
\begin{remark}
	\Cref{th:pvMD} shows that PV rules such that $\max_i v_i ≤ \frac{m + 1}{3}$ satisfy MD. One may wonder whether such rules necessarily satisfy a stronger version of MD that mandates picking \emph{all} minimal dispersion alternatives (thus $\argmin_{\PE} (d \circ \lprof) \subseteq f(\prof)$). This is false. To illustrate, pick $m = 5$ and consider the rule $PV^{(2, 2)}$ and the following profile $\prof$.
	\begin{equation}
		\begin{array}{*5{l}}
			a& b & c & d & e \\
			c& d & e & b & a\\
...
\end{remark}
\commentOC{I find it not nice that this remark takes more than 1 § (hence the reader doesn’t know where it ends by simple scanning of white space). Do we mind?}
...
The following theorem shows that satisfying MD does not imply being a Pareto-and-veto rule.

\begin{theorem}
	When $m ≥ 4$, $\MDcl \nsubseteq \PVcl$.
\end{theorem}
\begin{proof}
	Consider the rule $f(\prof) = \argmin_{\PE}(d \circ \lprof)$. It satisfies MD by definition. Let us show that it is not in $\PVcl$.

	Let $\allalts = \set{a, b, c, a_4, a_5, …}$.
	Consider the following profile $\prof$, also used in the proof of \cref{th:PVnotSPEL}:
	\begin{equation}
		\begin{array}{*6{l}}
			a&b&c&a_4&a_5&…\\
			c&b&a&a_4&a_5&…\\
		\end{array}.
	\end{equation}
	Observe that $f(\prof) = \set{b}$. To have $f \in \PVcl$ requires that $v_1 ≥ m - 2$ (for $c \notin f(\prof)$) and $v_2 ≥ m - 2$ (for $a \notin f(\prof)$), which implies $2m - 4 ≤ \sum v_i$. Also, the definition of $\PVcl$ requires that $\sum v_i ≤ m - 1$. And $2m - 4 ≤ m - 1$ is satisfied only when $m ≤ 3$.
\end{proof}
This result highlights the impossibility of characterizing MD in terms of PV complemented with other properties.
\commentOC{This observation might help replying to Jean-François’s suggestion (or not), but I am not sure that it is useful to the reader who maybe didn’t wonder about this possibility (though I don’t mind writing it anyway). About Jean-François’s suggestion, I think it was rather about finding X such that [MD + X] iff [$\PVcl \land v_i ≤ \frac{m + 1}{3}$].}
