...
In Proposition 11 \commentRS{Insert an auto citation.}below, we write $m=3k+r$ for $k\in \mathbb{N}$, $r\in\{0,1,2\}$ and let $\langle \frac{m}{3} \rangle=k$ for $r\in\{0,1\}$ while $\langle \frac{m}{3} \rangle=k+1$ for $r=2$.
...
\end{proposition}
\commentRS{I tried to make the necessity part more compact, I hope it is not too compact. We'll discuss the sufficiency part face to face.}
...
where $PE(\prof")=\{a_{1}, a_{2k+1}, x\}$ while
$d(\lambda_{\prof"}(a_{1})) =2k+2$, $d(\lambda_{\prof"}(a_{2k+1})) = k+2$ and $d(\lambda_{\prof"}(x)) = k$. Thus, any SCR that satisfies MD must pick $x$, which is, however, vetoed by individual.

\end{proof}

\begin{remark}
   As MD implies PEL, the class of PV rules that satisfy MD is a subclass of those that satisfy PEL. This relationship can be more precisely observed by comparing Propositions 6 and 11. \commentRS{Insert an auto citation for both propositions.} 
\end{remark}
