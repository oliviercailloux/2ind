...
\begin{definition}[$k$-Rawlsianism] 
	\label{def:kr}
	Given $k \in \intvl{0, m - 1}$, a SCR is $k$-Rawlsian iff it it prefers alternatives whose losses are within $\intvl{0, k}$ for both individuals. Formally:
	$\forall \prof \in \allprofs,  x, y \in \allalts, \lprof(x) \in \intvl{0, k}^2 \land \lprof(y) \notin \intvl{0, k}^2 ⇒ y \notin f(\prof)$.
\end{definition}
The property $k$-Rawlsianism, for which we thank Miguel Ballester, is binding at exactly those profiles where some alternative has its losses in $\intvl{0, k}^2$, and is void at the other profiles. It is thus natural to wonder which values of $k$ make the constraint binding at every profile, so as to guarantee a maximal possible loss to every individual \emph{a priori} (meaning, whatever the profile).
It follows from \citet[Theorem 1]{BramsKilgour2001} that $k$-Rawlsianism is binding if, and only if, $k$ is at least “half”.
\begin{theorem}
...
\begin{remark}
	...
	\commentOC{We can choose whether we thank him in the text (as above) or in a footnote (as here), with or without further details.} 
	Formally:
