...
\title{Two principles for two-person social choice \thanks{We would like to thank Sylvain Béal for his comments on an earlier draft of this paper as well as the seminar audience at CRESE.}}
...
\Citet{cailloux2022compromising} propose a different conception of compromising based on an \textit{equal loss principle} that favors outcomes where every individual concedes as equally as possible from his highest ranked alternative. They show that several two-person social choice rules fail one variant of it.
...
\begin{itemize}
	...
	\item The \textit{veto-rank mechanism} where, given an odd number $m$ of alternatives, each individual vetoes $(m - 1) / 2$ alternatives and ranks the remaining $(m+1) / 2$. The outcome is the alternatives with the minimal sum of ranks among those that have not been vetoed.
...
\begin{table}
	\begin{tabular}{l*{6}{c}}
		...
		$\PVe$ & ✓ & & & & ✓
	\end{tabular}
	\caption{Summary of the results. A tick means that all members of the corresponding class of rules satisfy the corresponding property (for $m ≥ 4$). The notation $\PVcl^=$ refers to the class referred to by \cref{th:pvmr}.}
	\label{fig:props}
\end{table}
...
\commentRS{This part should be slightly elaborated.}
