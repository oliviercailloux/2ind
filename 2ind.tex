...
Both $\VR$ and $\SL$ are defined in Clippel et al.XXX for $m$ odd only.
...
\commentRS{Define here $\PVe$ (égaux et le plus grand possible, thus $v_i = \floor{\frac{m - 1}{2}}$). When m is odd each vi is (m-1)/2 (binding) and when m is even each vi is (m/2)-1 (not binding)}
...
\end{proposition}
\begin{proof}
    \commentOC{Note that $\bigcup(\PEcl \cap \FHcl)$ is, by definition, the rule that, for each profile, picks all and only the pareto and first half alternatives. We thus have to show that $\forall \prof: \PVe(\prof) = \PE \cap H(\prof)$. By definition of $\PVe$, suffices to prove that $\cap_i\set{x \in \allalts \suchthat \lprof(x)_i ≤ m - \floor{\frac{m - 1}{2}} - 1} = H(\prof)$. This in turn follows from the definition of $H(\prof)$.}
    
Take any preference profile P. We first show 	$\bigcup(\PEcl \cap \FHcl) \subset \PVe$.
We now show 	$\bigcup(\PEcl \cap \FHcl) \supset \PVe$.
