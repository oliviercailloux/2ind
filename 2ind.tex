...
\begin{proof}
	...
	As $m - 1 - v_i \in \N$, $m - 1 - v_i ≥ \frac{2m - 4}{3} ⇒ m - 1 - v_i ≥ \ceil{\frac{2m - 4}{3}}$, and when $m ≥ 3$, $\ceil{\frac{2m - 4}{3}} ≥ \khalf$ \commentOC{The gory details: when $m$ odd, $m ≥ 5 ⇒ \frac{2m - 4}{3} ≥ \frac{m - 1}{2} = \khalf$ and when $m$ is even, $m ≥ 8 ⇒ \frac{2m - 4}{3} ≥ \frac{m}{2} = \khalf$; when $m = 3$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{2}{3}} = 1 = \khalf$; when $m = 4$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{4}{3}} = 2 = \khalf$; when $m = 6$, $\ceil{\frac{2m - 4}{3}} = \ceil{\frac{8}{3}} = 3 = \khalf$}.
	...
	If $v_i > \frac{m + 1}{3}$ for some $i \in \set{1, 2}$, then $m - 1 - v_i < \frac{2m - 4}{3}$. Say $i = 1$ (the rest of the argument applies symmetrically to the case $i = 2$).
	By \cref{th:caractEmpty}, $\exists \prof \suchthat \lprofinv(\intvl{0, m - 1 - v_1} × \intvl{0, m - 1}) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$. \commentOC{Except for $t$ too small where it holds but does not follow from \cref{th:caractEmpty}.}
	It follows from $f(\prof) \subseteq \lprofinv(\intvl{0, m - 1 - v_1} × \intvl{0, m - 1})$ that $f(\prof) \cap \argmin_{\PE} (d \circ \lprof) = \emptyset$.
\end{proof}
\commentOC{The previous version says that for $m = 5$, max is $1$, the new version says that for $m = 5$, max is $2$.}
