...
\Cref{sec:rev} reviews the literature, \Cref{sec:basic} introduces the basic notions and notations and  \Cref{sec:lit} defines the rules we analyze.
\Cref{sec:minprinc,sec:eqprinc} are devoted to the minimal Rawlsian and equal loss principles, respectively. \Cref{sec:reconc} introduces the Rawlsian minimal dispersion principle. \Cref{sec:concl} makes some concluding remarks.
...
Let $\PEcl = \set{f: \allprofs → \powersetz{\allalts} \suchthat \forall \prof \in \allprofs: f(\prof) \subseteq \PE}$ denote the class of SCRs satisfying the Pareto property.\footnote{We systematically use calligraphic letters to denote the class of rules satisfying a given property.}

In concordance with the ceiling established by Theorem 1 of \citet{BramsKilgour2001}, we use  “best half” to refer to the loss values up to $\khalf$, where $\lfloor x \rfloor$ denotes the greatest integer less than or equal to $x$. Accordingly, $\forall \prof \in \allprofs$, we let $H(\prof) = \set{x \in \allalts \suchthat \lprof(x) ≤ (\khalf, \khalf)}$ denote the set of alternatives reaching the best half of every individual’s preference, and $H^i(\prof) = \set{x \in \allalts \suchthat \lprof(x)_i ≤ \khalf}$ those that reach the best half of $i$’s preference. Given $\prof$ and a loss level $k$, define $U(\prof, k) = \set{x \in \allalts \suchthat \max \lprof(x) ≤ k}$ as the alternatives with a loss of at most $k$ for both individuals.
...
\begin{remark}
  \label{th:FBH}
  $\FB$ is anonymous and neutral. It satisfies the Pareto property. Moreover, $\forall \prof \in \allprofs$, $\FB(\prof) \subseteq H(\prof)$. The final observation comes from the fact that $\forall \prof \in \allprofs$, $\min_{x \in \allalts} \set{\max \lprof(x)} ≤ \khalf$, a consequence of the following proposition that is a restatement of \citet[Theorem 1]{BramsKilgour2001}. We include a proof for completeness.
\end{remark}
...
\begin{proposition}[\citeauthor{BramsKilgour2001}]
  \label{th:BKkhalf}
  $[\forall \prof \in \allprofs, \exists x \in \allalts \suchthat \max \lprof(x) ≤ k] ⇔ k ≥ \khalf$.
\end{proposition}
\normalsize
\begin{proof}
...
\begin{remark}
  \label{th:VRSLH}
  Both	$\VR$ and $\SL$ are anonymous and neutral. They satisfy the Pareto property. Moreover, $\forall \prof \in \allprofs$, $\VR(\prof) \subseteq H(\prof)$ and $\SL(\prof)\subseteq H(\prof)$.
\end{remark}
...
\begin{theorem}\label{th:intersection}
  $\forall \prof \in \allprofs: \PVe(\prof)=H(\prof)\cap PE(\prof)$.
\end{theorem}
\begin{proof}
...
\begin{proof}
  ...
  To show the “if” part, note that $\forall \prof \in \allprofs$, the condition $\forall i: v_i ≥ \floor{\frac{m - 1}{2}}$ suffices to guarantee that $\PVv(\prof) \subseteq H(\prof)$.
...
\begin{lemma}
  \label{th:sumlosses}
  $\forall \prof \in \allprofs$, if $x \in \PE$ then $\lprof(x)_1+\lprof(x)_2 \leq m-1$.
\end{lemma}
\begin{proof}
...
\begin{proposition}
  \label{th:pel}
  For any $m ≥ 2$, a rule $\PVv[(v_1, v_2)] \in \PVcl$ is PEL-compatible iff its veto levels are both at most $\floor{\frac{m}{2}}$, thus, iff $\max_{i \in \set{1, 2}} v_i ≤ \frac{m}{2}$.
\end{proposition}
\begin{proof}
  We have that $\forall \prof \in \allprofs$, if $x \in S(\prof) \cap \PE$, then $x$ is not among the last $\floor{\frac{m}{2}}$ ranks, as follows from \cref{th:sumlosses}.
...
\begin{corollary}
  \label{th:pvbpel}
  For any $m ≥ 2$, a rule $\PVv[(v_1, v_2)] \in \PVbcl$ is PEL-compatible iff $v_i = \floor{\frac{m - 1}{2}}$ and $v_{\ibar} = \khalf$ for any $\set{i, \ibar} = \set{1, 2}$.
\end{corollary}
...
\begin{proof}
  ...
  \begin{lemma}
    \label{th:PNonEmpty}
    Given $m ≥ 3$, $\forall \prof \in \allprofs: \lprofinv(\intvlz{\ceil{\frac{2m - 4}{3}}} × \intvlz{\ceil{\frac{2m - 4}{3}}}) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset$.
  \end{lemma}
  \begin{proof}
  ...
  \begin{lemma}
    ...
    \begin{enumerate}
      ...
      \item \label{it:Pt} $\forall \prof \in \allprofs: \lprofinv(\intvlz{t} × \intvlz{t}) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset$.
    \end{enumerate}
  \end{lemma}
  ...
  \begin{lemma}
    ...
    $\forall \prof \in \allprofs: H(\prof) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset ⇔ m ≤ 6 \lor m = 8$
  \end{lemma}
  \begin{proof}
    ...
    \begin{equation}
      m ≤ \frac{3 \khalf + 4}{2} ⇔ \forall \prof \in \allprofs: H(\prof) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset.
    \end{equation}
...
\begin{proof}
  ...
  By definition, $\forall \prof \in \allprofs: \PVv[(v_1, v_2)](\prof) = \lprofinv(\intvlz{m - 1 - v_1} × \intvlz{m - 1 - v_2}) \cap \PE$.
  It follows that $\forall \prof \in \allprofs$:
  \begin{equation}
  ...
  It follows from \cref{th:caractEmpty,th:PEmpty} that $t ≥ \frac{2m - 4}{3} ⇔ \forall \prof \in \allprofs: \lprofinv(\intvlz{t} × \intvlz{t}) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset ⇔ \forall \prof \in \allprofs: \lprofinv(S) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset$, and thus $\max_{i \in \set{1, 2}} v_i ≤ \frac{m + 1}{3} ⇔ t ≥ \frac{2m - 4}{3} ⇔ \forall \prof \in \allprofs: \PVv[(v_1, v_2)](\prof) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset$.
\end{proof}
...
\begin{proposition}
  \label{th:dispbound}
  $[\forall \prof \in \allprofs, \exists x \in \PE \suchthat d(\lprof(x)) ≤ k] ⇔ k ≥ \khalf$.
\end{proposition}
\begin{proof}
  \Cref{th:intersection} establishes, $\forall \prof \in \allprofs$, the existence of an alternative $x \in \PE \cap H(\prof)$. Thus $d(\lprof(x)) ≤ \khalf$, proving the “if” part. To show the “only if” part, consider the profile $\prof = (\pref_1, \pref_2)$ defined as
  \begin{equation}
...
\end{proposition}
\begin{proof}
  First consider $[m ≤ 6 \lor m = 8] ⇒ \RMDcl \subseteq \MDcl$. \Cref{th:profsIncompat} indicates that $\forall \prof \in \allprofs: H(\prof) \cap \argmin_{\PE} (d \circ \lprof) ≠ \emptyset ⇔ m ≤ 6 \lor m = 8$.
...
It also follows from \cref{th:equal} that $\PVe$ fails Strong RMD. Thus, no rule that we have examined so far satisfies Strong RMD. Now, we define the Strong RMD rule as the least resolute rule satisfying Strong RMD, namely, $\forall \prof \in \allprofs: f(\prof) = \argmin_{H(\prof) \cap \PE} (d \circ \lprof)$.
...
$\forall \prof \in \allprofs$, $E(\prof)$ denotes the alternatives whose loss vectors are not dominated: $E(\prof) = \set{x \in \allalts \mid \nexists y \in \allalts: \lprof(y) ⊲ \lprof(x)}$.
...
\begin{table}
  ...
  \caption{Summary of the results about $\FB$, $\VR$ and $\SL$.
    A tick (✓) means that the SCR satisfies the corresponding property for any $m$.
   }
  \label{fig:propsR}
\end{table}
...
\begin{table}
  \begin{tabular}{l*{5}{c}}
    ...
    \bottomrule
  \end{tabular}
  \caption{Summary of the results about the class $\PVcl$ when $m\geq 4$. Each cell specifies those members that satisfy the corresponding property.}
