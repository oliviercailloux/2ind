...
\begin{equation*}
        ...
        \end{array}.
    \end{equation*}
    Whereas $c$ is the only alternative satisfying the equal loss principle, only $a$ and $b$ are Pareto efficient. This incompatibility is formally stated by the next proposition. 
\begin{proposition}
    $\forall m ≥ 3: \PEcl ∩ \ELcl = \emptyset$.
\end{proposition}
\commentRS{Proposition 4 is missing a proof (that is rather straightforward).}\commentMN{Just added it, please check that you agree}
...
\commentOC{Add a remark to discuss this theorem verally, and point out that 2 eq 3 can also be seen more directly, just by anonymity.}
...
\commentRS{As Olivier noted, we have to choose whether we keep the above approach or the one below. As the impossibility is stronger with the weaker definition of MD, I am inclined to keep the weak one. To be sure, the proposition below adds something to corollary 4 (that we discussed by mail) but the this additional contribution is rather marginal and I don't think it is worth to define strong MD just for this. Thus, I suggest discarding Proposition 9.}\commentMN{Let's discard Prop 9 then}
