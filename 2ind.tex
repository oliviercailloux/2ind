...
Any rule $f$ can be viewed as a combination of two principles: one that gives required conditions for being among the set of winners; and one that gives required conditions for not being among the set of winners; or equivalently, one “necessity” principle (having the form $f(\prof) \subseteq g(\prof)$ for some $g: \allprofs → \powersetz{\allalts}$) and one “sufficiency” principle (having the form $g(\prof) \subseteq f(\prof)$. We apply this observation, together with the observation that $\FB$, $\VR$, $\SL$ and $\PVe$ satisfy $FH$ to obtain simple characterisations of these four rules. (We obviously do not claim that these characterisations are particulary ingenious, as they mainly reflect the principles that have been adopted when designing them; but we think that they summarize efficiently some of the above observations and permit to compare these four rules easily.)

The FH property is the “necessity” principle for all four rules: $f(\prof) \subseteq H(\prof)$.

Our first “sufficiency” principle is the least demanding of all four: any “first half” alternative wins. Formally: $H(\prof) \subseteq f(\prof)$. FH and this principle together characterise $\PVe$.

Another sufficiency principle demands that any alternative that is not FH or that is dominated by an FH alternative in terms of maximal welfare loses. FH and this principle characterise $\SL$. This principle can also be described as follows. Define $≥$ as the following preorder on the set of loss vectors: $(a, b) ≥ (c, d) ⇔ \min\set{a, b} ≥ \min\set{c, d} \lor c ≥ half \lor d ≥ half$. Then, the sufficiency principle of $\SL$ is that if $x$ is a maximal element among the loss vectors that exist in $\prof$, then $x \in f(\prof)$; formally, $\argmax^{≥}_{x \in \allalts} \lprof(x) \subseteq f(\prof)$.
