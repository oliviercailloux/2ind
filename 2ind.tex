...
\begin{remark}
	This theorem states several results that will be useful throughout and that may help illustrating the behavior of the rules we consider. First, it states (considering $1 ⇔ 2$) that if one wants to place a limit to the level at which we may look “down the profile”, for both players, then it is necessary and sufficient to place this limit at $t$ being at least $\frac{2m - 4}{3}$ to obtain that one of the alternatives in the “upper part” of the profile is among the minimizers of dispersion (for any profile).
