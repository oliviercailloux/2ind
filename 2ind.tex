...
In concordance with the ceiling established by Theorem 1 of Brams and Kilgour (2001) \commentOC{To cite (and other such refs)}, we use the term “half” to mean the smallest integer $k$ that exceeds $m-k$.
...
\end{definition}
Let $\PEcl$ denote the class of rules picking only Pareto efficient alternatives ($\forall \prof: f(\prof) \subseteq \PE$); $\FHcl$ denote the class of rules satisfying the First half property \commentOC{First half or first half? (I prefer First half to distinguish from the informal phrase.)}, and similarly for other properties defined in this article.
...
\end{proposition}
\begin{proof}
	This follows immediately from the definition of these rules. \commentOC{It follows literally immediately for $\VR$, and for the other two, it is an easy consequence, considering what we wrote above. Is a proof required?}
\end{proof}
\commentRS{Define here PVe for m odd and even separately. When m is odd each vi is (m-1)/2 and when m is even each vi is (m/2)-1}
\commentRS{I suggest to state the following proposition in two parts: when m is odd, the intersection of the two classes PV and FH is a singleton that contains PVe. When m is even, this intersection contains three rules, namely PVe, one where v1=(m/2)-1, v2=m/2 and a third where v1=(m/2), v2=(m/2)-1. The proof will exploit 	$\PVcl ∩ \FHcl = \set{\PVv \in \PVcl \suchthat v_i \geq \floor{\frac{m - 1}{2}}, \forall i }$.}
\begin{proposition}
	$\PVcl ∩ \FHcl = xxxxx$. 
\end{proposition}
\begin{proof}
	...
	Observe that, for each $i$, $\PVv(\prof)$ will pick winners exclusively in the first “half” of voter $i$ exactly when $v_i ≥ \floor{\frac{m - 1}{2}}$. \commentOC{Formally: $[x \in \PVv(\prof) ⇒ \lprof(x)_i ≤ \ceil{\frac{m - 1}{2}}] ⇔ v_i ≥ \floor{\frac{m - 1}{2}}$.}
