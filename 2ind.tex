...
\end{proposition}
It follows that the strongest satisfiable version of $k$-Rawlsianism is when $k$ equals $\khalf$. On the other hand, this choice of $k$ reflects a general bound that makes $k$-Rawlsianism satisfiable at every preference profile while there are several profiles where the minial loss is lower than $k$. Thus, we qualify $\khalf$-Rawlsianism as “minimal Rawlsianism” which we formally define as follows:
...
\end{proof}
\commentRS{What is the connection between Theorem 3 and Theorem 5.1 in our paper with Beatrice?}
\begin{proposition}
...
\commentRS{Perhaps we can call Proposition 9 a remark. I suggest to start the proof of Theorem 3 by saying that we will indeed show that $H(\prof) \cap \argmin_{\PE} (d \circ \lprof)$ is empty. So, Proposition 9 (or the Remark) can simply state and show the non-emptiness of $H(\prof) \cap \argmin_{\PE} (d \circ \lprof)$.}The minimal Rawlsian and minimal dispersion principles being logically incompatible, the SCRs that satisfy MR (namely FB, VR, SL and the PV rules identified by Theorem 2) all fail MD. As Theorem 2 shows that most PV rules fail MR, the following result which determines the class of PV rules satisfying MD is of interest. 
\commentOC{We could have first a technical result about the emptyness (or not) of the intersection depending on the value of $m$; then Theorem 3 and proposition 9 as corollaries.}
\begin{proposition}
...
\end{proposition}
\commentRS{I'll send a hand-written sketch of the proof.}\commentMN{I added the proof by Remzi }


\begin{proof}
Let $m=3k+r$ for some $k\in 
\mathbb{N}$.
...
The necessity part. Suppose $v_{i}>$ $\floor{ \frac{m}{3}}$
...
There are 3 Pareto efficient alternatives, $a_{1}$, $a_{2k+1},a_{2k}$ whose
respective dispersions are $k+1$, $2k$, $k-1$. Thus, a rule that satisfies
MD must pick $a_{2k}$ which is vetoed by individual 1.
...
There are 3 Pareto efficient alternatives, $a_{1}$, $a_{2k+1},$ $x$ whose
respective dispersions are $k+1$, $2k+1$, $k$. Thus, a rule that satisfies
MD must pick $x$ which is vetoed by individual 1.
...
There are 3 Pareto efficient alternatives, $a_{1}$, $a_{2k+1},$ $x$ whose
respective dispersions are $k+2$, $2k+2$, $k$. Thus, a rule that satisfies
MD must pick $x$ which is vetoed by individual 1.
