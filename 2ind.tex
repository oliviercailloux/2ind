...
$[\exists i \suchthat v_i = \floor{\frac{m - 1}{2}} \land \floor{\frac{m - 1}{2}} ≤ v_{\ibar} ≤ \khalfAlt = \khalf]$. Thus, the claim is equivalent to the claim $\PVcl ∩ \MRcl = \set{\PVv \in \PVcl \suchthat \forall i: v_i ≥ \floor{\frac{m - 1}{2}}}$, which we now prove.
	...
	To show the “if” part, note that given any profile, the condition $\forall i: v_i ≥ \floor{\frac{m - 1}{2}}$ suffices to guarantee that $\PVv(\prof) \subseteq H(\prof)$. 
...
\begin{table}
	\begin{tabular}{cl*{5}{c}}
		...
		\bottomrule
	\end{tabular}
	\caption{Summary of the results. \commentOC[A tick means that]{Is that an iff?} all members of the corresponding class of rules satisfy the corresponding property (for $m ≥ 4$). \commentOC{Actually some of the non-ticked cells of the table create difficulties because some rules satisfy or fail some properties depending on the value of $m$. It seems to me that taking this into account would make the table less readable. Therefore, I suggest to simply leave it as it is now, that is, not stating anything about the empty cells.}
