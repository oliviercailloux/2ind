...
\commentOC{I indicated the relationship with $\PVv$, is this appropriate or confusing?}
...
\commentOC{(Very minor.) Shall we rather write “Note that $PV^=\in\PVbcl$ iff $m$ is odd.”?}
...
\begin{proof}
	...
	\commentOC{This equivalence is incorrect, isn’t it?}
...
\end{proof}
A direct implication of Proposition 1 \commentRS{Can't we cite Proposition 1 automatically so that if the numbering changes the citation also changes}  is that the set of $\PVcl$ rules satisfying $\FHcl$ is quite reduced, as made explicit by the following remark.
\begin{remark}
...
\commentOC{(Very minor.) Could we rather write: “When $m$ is even, $\PVcl ∩ \FHcl = \set{\PVe, \PVv[(\frac{m}{2}, \frac{m}{2} - 1)], \PVv[(\frac{m}{2} - 1, \frac{m}{2})]}$”?}
\end{remark}
...
\end{proposition}
\begin{proof}
    Note that $\bigcup(\PEcl \cap \FHcl)$ is, by definition, the SCR that, for each profile, picks all Pareto alternatives that are in the first half of both players’ preferences and only those alternatives. \commentOC{Seems like some game theorist is hacking our paper.}
...
The two observations below follow from Proposition 2. \commentRS{Can't we cite Proposition 2 automatically so that if the numbering changes the citation also changes} 
\begin{corollary}
	A SCR $F \in \PEcl \cap \FHcl$ if and only if $F\subseteq \PVe$.
	\commentOC{Shall we write $f$?}
\end{corollary}
 
\begin{corollary}
   	$\FB, \VR, \SL \subset \PVe$.
   	\commentOC{This requires to ensure that these three rules differ from $\PVe$. Is this sufficiently obvious that we don’t even state it? (I don’t mind, just asking.) Also, this seems to require both Prop. 2 and Remark 3, so perhaps we need to rephrase anyway.}
\end{corollary}
