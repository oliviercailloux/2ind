...
\end{definition}
\commentRS{I revised the wording of the definition above.} 
$k$-Rawlsianisn is binding at exactly those profiles where some alternative has its losses in $\intvl{0, k}^2$, and is void at the other profiles. \commentRS{I deleted the thanks to Ballester, I think we should do this somewhere else.} It is thus natural to wonder which values of $k$ make the constraint binding at every profile, so as to guarantee a maximal possible loss to every individual \emph{a priori} (meaning, whatever the profile). \commentRS{minimal possible loss, no? Let's not use a priori.} 
