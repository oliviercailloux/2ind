...
\commentMN{Remark that Strong RMD (hence RMD) does not imply MD since the unique Pareto efficient alternative of the profile that minimizes dispersion need not be in the first half.  Consider for example the profile such that
	\begin{equation}
		\begin{array}{lllllllll}
			a_1&\ldots&a_{p-1}&x&a_{p+1} & \ldots&\ldots&\ldots   &a_{2p+1} \\
			a_{p+1}&\ldots&\ldots &a_{2p} &a_{2p+1}&x&a_1&\ldots&a_{p-1}
		\end{array}.
	\end{equation}
Remark that $x$ is Pareto efficient while not in the FH. The losses associated to $x$ equal $(p-1,p+1)$ and hence the dispersion is $2$. Simple inspection of the profile shows that $x$ is indeed minimizing dispersion. Finally, $a_{p+1}$ is the only alternative in the FH (that is with losses less than $p$) showing that strong RMD does not imply MD.}
...
\commentOC{I do not understand what the wrapping of my previous § within a Matias’ comment means. Do you suggest to remove the sentence? Shouldn’t it be replaced by something, then? Also, I believe that the word “strict” (which was after Paretian) should remain there otherwise the conclusion doesn’t necessarily hold.}
  ...
  Yet, a strong RMD rule must pick within the  “first half” alternatives $\set{a, d}$.
	\commentOC{It might be interesting to prove that $\forall m$, Strong RMD is not stronger than MD}.\end{remark}
...
\commentMN{Maybe we need a better name than Strong RMD. What about the REF function? REF as Rawlsian (MR), Efficient (Pareto efficient) and Fair ( minimal dispersion)}
...
\begin{conjecture}
	\commentMN{The Strong RMD rule is well defined and contains at most two alternatives.}
\end{conjecture}
\commentOC{I am favorable to only write the conjecture as “The Strong RMD rule contains at most two alternatives” rather than “The Strong RMD rule is well defined and contains at most two alternatives”: the proof does not prove that it is well defined. That part is considered self-evident even from the definition, otherwise it should not be called “rule”.}
\begin{proof}
