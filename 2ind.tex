...
\end{remark}
The relationship between the minimal dispersion principle and Pareto-and-veto rules is not closer than the proximity suggested by \cref{th:pvMD}, which can be observed by the two remarks below:  
\begin{remark}
...
\begin{remark}
Satisfying MD does not imply being a Pareto-and-veto rule. To see this, consider the rule $f(\prof) = \argmin_{\PE}(d \circ \lprof)$ which satisfies MD by definition but is not in $\PVcl$ as the following example shows:
