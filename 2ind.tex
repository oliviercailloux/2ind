...
\commentMN{This property allows to distinguish FB from the other rules that we consider. This stronger version requires that $f$ selects uniquely the Pareto efficient alternatives that satisfy equal loss.}
...
\subsection{A caracterisation of FB based on weight comparison}
For any $w_m, w_i \in [0, 1]$ with $w_m + w_i = 1$, let $f^{w_m, w_i}$ denote the rule that selects the maximal elements according to the disutility of the loss vectors weighted by $w_m$ and $w_i$, as follows: $f(\prof) = \argmax_{x \in \allalts} -w_m(\min\lprof(x)) - w_i(\max \lprof(x) - \min \lprof(x))$. \commentMN{Why do we have $\max \lprof(x) - \min \lprof(x)$?? Maybe add Remark 6 here and mention that Borda satisfies it. It is not simple to understand this point. Actually, the class of these rules is not immediate to me.}
...
\begin{proof}
	...
	Indeed, $\forall y \in \allalts$, letting $y_m$ denote $\min \lprof(y)$ and $y_i$ denote $\max \lprof(y) - \min \lprof(y)$, we obtain that: 
	
	\begin{align*}
	    y \in f(\prof) \text{ only if }& -w_m \min \lprof(y) - w_i (\max \lprof(y) - \min \lprof(y)) ≥ -w_m k,\\
	    &\Longleftrightarrow w_m (k - \min \lprof(y)) \geq w_i (\max \lprof(y) - \min \lprof(y)) \\
	    &\Longrightarrow k - \min \lprof(y) \geq  \max \lprof(y) - \min \lprof(y) \: (\:\text{since } w_i \geq w_m \:)\\
	    &\Longleftrightarrow k \geq  \max \lprof(y).
	\end{align*}
This requires that $y$ Pareto-dominates $x$, and since $x \in \PE$, entails that $y = x$.
\end{proof}
