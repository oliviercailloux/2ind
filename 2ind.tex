...
\commentMN{This notation can be improved, what about $\lprof^{min}(x)$?} 
...
\commentOC{We think that perhaps we can keep the notation as is for the moment, but we can talk about it.}
...
\commentOC{I suggest to use $\lprof^{-1}(\intvl{0, k - 1} × \intvl{0, k - 1})$ instead of $U(\prof, k)$.}
...
\commentOC{Here we could use our previously defined notation.}
...
\commentOC{Perhaps here we could substitute the definition of $\rho$, otherwise the reader will almost surely have to seek upwards for its definition.}
...
\begin{proof}
	...
	\commentOC{Here as well, I propose to substitute the definition of $H(P)$ (and perhaps recall the definition of $\PVv$).}
...
\commentOC{I’d find nice to write $\lprof^{-1}(\mathrel{=})$, treating $\mathrel{=}$ as a binary relation over integers, to denote the alternatives that are ranked at the same position by both individuals. We could attempt to justify this bold move with something like: We realize that this is a very unorthodox abuse of notation about the [or: our beloved] equal sign, but we feel this is justified in the present case as $\lprof^{-1}(\mathrel{=})$ literally then means “those alternatives having equal losses”. (But I realize this is perhaps too unorthodox so I won’t complain if you don’t like it.)}
...
\end{definition}
\commentOC{I now understand this demand as promoting a sort of extreme individualism, not a love for egalitarianism. In the latter view (which I thought applied here), the normative basis for this principle seems doubtful. The principle mandates to pick an alternative that is PE and same-ranked as a winner, however comparably bad it is in terms of average rank. It seems arbitrary, if one thinks that the ground is the appreciation of egalitarianism, to enforce the principle \emph{whatever the cost} (in average rank) \emph{only} whenever the alternative is pareto (this restriction being unjustified for an extreme egalitarianist). In other words, the restriction accounts for some willingness to relax egalitarianism, which does not cohere well with the carelessness about the loss in terms of average rank, if talking about egalitarianism and not individualism. (The principle is perhaps better understood as one that takes individuals extremely seriously, however.)}
...
\begin{proof}
\commentOC{TODO!}
	For $m = 5$, consider the profile $abcde$ and $cbade$. Here, PEL requires to choose uniquely $b$. 
	But, if PV picks b, it also picks something else.
	In more details, consider any $f$ in $\PVcl$. If $b \notin f(\prof)$, the proposition holds. Assume $b \in f(\prof)$. We show that $f(\prof)$ includes another alternative. PV= chooses abc; PV(1, 3) picks ab and PV(3, 1) picks bc. 
\end{proof}
...
Thus, by satisfying both conditions, FB establishes the compatibility between PEL and MR. However, as discussed below, this compatibility vanishes when a stronger version of PEL is adopted. 

Call the dispersion of a loss vector $l$ at $\prof$ the value $d(l) = \abs{l_1 - l_2}$. 
Thus, $(d \circ \lprof)(x) = \max\lprof(x) - \min\lprof(x)$.
Note that $d \in \Sigma$ seems to coincide with multiple commonly used spread measures (to be verified). 
...
\commentOC{Is this really minimal? I think that name is deserved for $MD(1)$ only. What about up-to-$k$-minimal dispersion? Within-$k$ minimal dispersion? $k$-regret bounded dispersion? Or “small dispersion $k$”?}

\commentOC{Shouldn’t we rather mandate only that $f$ takes at least one of its winners among the ones that have bounded dispersion? This would strengthen the principle more gently (though still with $MD(1)$ and $MD(2)$ implying PEL) but would, I suppose, suffice for the incompatibility we seek.}
\commentOC{Independently of the previous comment, I wonder if we need $MD(k)$: we only seem to use $MD(1)$. Thus perhaps a “take at least one of the minimally dispersed” principle would be more economical? (I’m hesitant: the incompatibility is stronger in the current version than with only $MD(1)$ and that’s nice.)}
\begin{definition}[Minimal dispersion $k$]
...
3. Can we use MD(1) to refine PV, VR, SL or FB? Discuss refinements of these rules. The Theorem, stated at the end of page 4 (\hrefblue{https://link.springer.com/article/10.1007/BF00187429}{Sprumont}), is about refinements of FB. It says that FB admits refinements that satisfy the two conditions of the paper, namely CPB and SO. Moreover, any rule that satisfies CPB and SO is a refinement of FB. Here are a few examples that illustrate the point of the question. At the profile 
\begin{center}
	$
	\begin{array}{cccccc}
